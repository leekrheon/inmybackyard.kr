  ...
  useEffect(() => {
    ...
    function renderIntroSequence() {
      ...
      if (step1ContainerRef.current) {
        ...
        // 이미지 전체화면 확장 완료 후 배경색을 #22CD6D로 전환
        const bgProgress = mapRange(s.currentIntroProgress, 12, 17, 0, 1);
        const r = Math.round(34 * bgProgress);
        const g = Math.round(205 * bgProgress);
        const b = Math.round(109 * bgProgress);
        step1ContainerRef.current.style.backgroundColor = bgProgress > 0
          ? `rgb(${r}, ${g}, ${b})`
          : 'transparent';
      }
    ...
    <>
      ...
      <div ref={introScreenRef} id="intro-screen" className="fixed inset-0 bg-white z-[300] overflow-hidden no-select" style={{ display: 'block' }}>
        <div ref={step1ContainerRef} className="intro-layer flex items-center justify-center">
          <img ref={introTextRef as any} src="/imby.png" alt="IMBY" className="absolute w-[70vw] md:w-[55vw] object-contain" style={{ userSelect: 'none', filter: 'brightness(0)' }} />
