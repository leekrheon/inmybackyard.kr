...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'projects' | 'press' | 'contact' | null>('projects');
  ...
  useEffect(() => {
    ...
    function renderIntroSequence() {
      ...
      // ment1~3 페이드인/아웃
      if (typo1Ref.current) typo1Ref.current.style.opacity = String(Math.max(0, mapRange(s.currentIntroProgress, 16, 22, 0, 1) - mapRange(s.currentIntroProgress, 28, 34, 0, 1)));
      if (typo2Ref.current) typo2Ref.current.style.opacity = String(Math.max(0, mapRange(s.currentIntroProgress, 34, 40, 0, 1) - mapRange(s.currentIntroProgress, 50, 56, 0, 1)));
      if (typo3Ref.current) typo3Ref.current.style.opacity = String(Math.max(0, mapRange(s.currentIntroProgress, 56, 62, 0, 1) - mapRange(s.currentIntroProgress, 72, 78, 0, 1)));
      // info1: ment1과 함께 등장, ment2 등장 시 퇴장
      const info1El = document.getElementById('info1-img');
      if (info1El) info1El.style.opacity = String(Math.max(0, mapRange(s.currentIntroProgress, 16, 22, 0, 1) - mapRange(s.currentIntroProgress, 28, 34, 0, 1)));
      // info2: ment2와 함께 등장, ment3 등장 시 퇴장
      const info2El = document.getElementById('info2-img');
      if (info2El) info2El.style.opacity = String(Math.max(0, mapRange(s.currentIntroProgress, 34, 40, 0, 1) - mapRange(s.currentIntroProgress, 50, 56, 0, 1)));
      ...
      // boy.png — progress 0~12: 위에서 내려옴, 역스크롤 시 다시 올라감
      const boyEl = document.getElementById('boy-img');
      if (boyEl) {
        const boyP = mapRange(s.currentIntroProgress, 0, 12, 0, 1);
        const boyY = -220 + 220 * boyP;
        boyEl.style.transform = `translate(-50%, ${boyY}px)`;
        boyEl.style.opacity = String(mapRange(s.currentIntroProgress, 0, 6, 0, 1) * mapRange(s.currentIntroProgress, 10, 15, 1, 0));
      }
    ...
    <>
      ...
      <div ref={introScreenRef} id="intro-screen" className="fixed inset-0 bg-white z-[300] overflow-hidden no-select" style={{ display: 'block' }}>
        <div ref={step1ContainerRef} className="intro-layer flex items-center justify-center">
          <div style={{ position: 'absolute', top: '47%', left: '50%', transform: 'translate(-50%, -50%)', width: 'clamp(300px, 65vw, 700px)' }}>
            ...
            <img ref={introTextRef as any} src="/imby.png" alt="IMBY"
              ...
              style={{ userSelect: 'none', filter: 'brightness(0)', display: 'block' }} />
          </div>
        ...
        <div ref={step2ContainerRef} className="intro-layer" style={{ pointerEvents: 'none' }}>
          {/* ment1 + info1 */}
          <div style={{ position: 'absolute', top: '12%', left: '50%', transform: 'translateX(-50%)', width: 'clamp(280px, 70vw, 820px)' }}>
            <img ref={typo1Ref} src="/ment1.png" alt="ment1"
              className="w-full object-contain"
              style={{ opacity: 0, display: 'block', filter: 'brightness(0)' }} />
            <img ref={typo2Ref} src="/ment2.png" alt="ment2"
              className="w-full object-contain"
              style={{ opacity: 0, display: 'block', filter: 'brightness(0)', position: 'absolute', top: 0, left: 0 }} />
          </div>
          {/* info1 */}
          <img id="info1-img" src="/info1.png" alt="info1"
            style={{
              position: 'absolute', bottom: 0, right: '5%',
              height: 'clamp(300px, 58vh, 680px)', width: 'auto',
              objectFit: 'contain', opacity: 0,
              pointerEvents: 'none', display: 'block',
            }} />
          {/* info2 */}
          <img id="info2-img" src="/info2.png" alt="info2"
            style={{
        ...
        <div ref={step4ContainerRef} className="intro-layer" style={{ pointerEvents: 'none' }}>
          <div style={{ position: 'absolute', top: '12%', left: '50%', transform: 'translateX(-50%)', width: 'clamp(280px, 70vw, 820px)' }}>
            <img ref={typo3Ref} src="/ment3.png" alt="ment3"
              className="w-full object-contain"
              style={{ opacity: 0, display: 'block', filter: 'brightness(0)' }} />
          </div>
        </div>
