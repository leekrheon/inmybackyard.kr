...
function MailIconWithBadge({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    // 0.8초 뒤 첫 알림, 이후 1.2초마다 1씩 증가 (최대 9)
  ...
  return (
    <div
      className={`mail-icon-intro${active ? ' active' : ''}`}
      style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 'clamp(80px, 14vw, 140px)', pointerEvents: 'none', zIndex: 160 }}
    >
      <div style={{ position: 'relative', display: 'inline-block', width: '100%' }}>
        <img src="/mail.svg" alt="" style={{ width: '100%', display: 'block' }} />
        {/* 알림 뱃지 */}
        {started && count > 0 && (
          <div
            key={count}
            className={popping ? 'badge-pop' : ''}
            style={{
              position: 'absolute',
              top: '-8%',
              right: '-8%',
        ...
        <div ref={step4ContainerRef} className="intro-layer" style={{ pointerEvents: 'none' }}>
          ...
          <div id="ment3-wrap" style={{ position: 'absolute', top: 0, right: 0, width: '50%', height: '100%',
            ...
            <img ref={typo3Ref} src="/ment3.svg" alt="ment3"
              style={{ width: '105%', objectFit: 'contain', objectPosition: 'left center', filter: 'invert(1)' }} />
          </div>
        </div>
