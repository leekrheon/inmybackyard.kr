    ...
    <>
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          /* 버튼 padding — 모바일은 글자(PNG) 크기를 키우기 위해 살짝 조정 */
          .glass-nav button { padding: 0.5rem 1.1rem !important; }
          /* PNG 높이 — 모바일에서 글자가 너무 작아 안 보이는 문제 → 확대 */
          .nav-label { height: 22px !important; width: auto !important; object-fit: contain !important; display: block !important; }
          /* 활성 탭(흰색 반전) 글자가 iOS Safari에서 안 보이는 문제 —
             backdrop-filter 부모 안에서 자식 filter(invert)가 렌더링 안 되는 버그 →
             모바일은 반전 없이 검정 유지 (초록 배경 위에서도 대비 충분) */
