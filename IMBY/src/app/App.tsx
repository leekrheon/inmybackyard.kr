  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 bg-black/90 hover:bg-black text-white px-5 py-3 rounded-full font-medium shadow-xl transition-all duration-300 hover:scale-105 active:scale-95"
            ...
            {articles.map(article => (
              ...
              >
                ...
                <div className="mt-5 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] w-[320px]">
                  <h3 style={{ fontFamily: "Pretendard, -apple-system, sans-serif", fontWeight: 700, fontSize: "1.2rem", color: "white" }}>{article.title}</h3>
...
export default function App() {
  ...
  if (view === 'article-editor') {
    return (
      <ArticleEditorPage
        ...
        onClose={() => { setEditingArticle(undefined); setView('site'); setTimeout(() => { if (glassNavRef.current) glassNavRef.current.classList.add('visible'); if (siteLogoRef.current) siteLogoRef.current.classList.add('visible'); const adminBtnWrapper = document.getElementById('admin-btn-wrapper'); if (adminBtnWrapper) adminBtnWrapper.classList.add('visible'); if (galleryWrapperRef.current) galleryWrapperRef.current.style.opacity = '1'; setActiveTab('article'); }, 50); }}
      />
    );
  }
  ...
  return (
    <>
      ...
      {/* Press Modal */}
      <div className={`fullscreen-modal fixed inset-0 z-[150] bg-[#fafafa] transition-transform duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] flex items-center justify-center ${activeTab === 'press' ? 'translate-y-0' : 'translate-y-full'}`}>
      ...
      {/* Contact Modal */}
      <div className={`fullscreen-modal fixed inset-0 z-[150] bg-[#fafafa] transition-transform duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] overflow-y-auto pt-32 pb-24 ${activeTab === 'contact' ? 'translate-y-0' : 'translate-y-full'}`}>
