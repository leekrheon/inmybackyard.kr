    ...
    >
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40 flex items-center gap-1 p-1.5"
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2 rounded-full font-medium transition-all duration-300 hover:scale-105 active:scale-95 text-black hover:bg-[#0FCD60] hover:text-white"
          style={{ cursor: 'pointer', fontSize: '0.88rem', letterSpacing: '0.04em' }}
        >
          {isViewAll
            ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
          }
          <span>{isViewAll ? '캔버스 뷰' : '전체 보기'}</span>
        </button>
        {/* 관리자: 새 아티클 */}
        {isAdmin && (
          <button
