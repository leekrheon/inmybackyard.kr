        ...
        <div className="w-72 shrink-0 border-r border-gray-100 overflow-y-auto p-6 flex flex-col gap-5">
          ...
          <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
            <input type="checkbox" checked={!!form.hidden}
              ...
              className="w-4 h-4 accent-[#0FCD60]" />
            <span className="text-gray-600">이 프로젝트 홈페이지에서 숨기기</span>
          </label>
          {form.hidden && (
            <p className="text-xs text-amber-600 -mt-3">숨김 상태에서는 일반 방문자에게 보이지 않아요. 언제든 체크 해제하면 다시 노출됩니다.</p>
          )}
  ...
  return (
    <>
                  ...
                  <div className="gallery-img-container relative" style={proj.hidden ? { opacity: 0.4 } : undefined}>
                    ...
                    {isAdmin && (
                      <div className="absolute top-2 right-2 flex flex-col gap-1.5 items-end">
                        ...
                        <button className="edit-btn" onClick={e => { e.stopPropagation(); handleToggleHidden(proj); }}>
                          {proj.hidden ? (
                            <><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7-11-7-11-7z"/><circle cx="12" cy="12" r="3"/></svg> 숨김 해제</>
                          ) : (
                            <><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M17.94 17.94A10.94 10.94 0 0112 20c-7 0-11-8-11-8a19.66 19.66 0 015.06-6.06M9.9 4.24A9.12 9.12 0 0112 4c7 0 11 8 11 8a19.5 19.5 0 01-2.16 3.19m-6.72-1.07a3 3 0 11-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg> 숨기기</>
                          )}
                        </button>
                      </div>
                    )}
      ...
      <div aria-hidden={selectedProjectIndex === null} className={`fullscreen-modal fixed inset-0 z-[200] bg-[#fafafa] transition-transform duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] overflow-y-auto pt-24 ${selectedProjectIndex !== null ? 'translate-y-0' : 'translate-y-full'}`}>
        ...
        {selectedProject && (
          <>
            {/* Hero: 제목 뒤로 크게 비치는 대표 이미지 + 하단 그라데이션으로 본문과 자연스럽게 이어짐 */}
            {selectedProject.img ? (
              <div className="relative w-full -mt-24 h-[58vh] md:h-[74vh] overflow-hidden">
                <img src={selectedProject.img} alt={selectedProject.title} className="absolute inset-0 w-full h-full object-cover" />
                {/* 제목 가독성을 위한 어둡게 처리 */}
                <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.7) 0%, rgba(0,0,0,0.25) 38%, rgba(0,0,0,0) 62%)' }} />
                {/* 하단부를 페이지 배경색(#fafafa)으로 자연스럽게 페이드시켜 본문으로 이어지게 함 */}
                <div className="absolute bottom-0 left-0 right-0 h-40 md:h-56" style={{ background: 'linear-gradient(to bottom, rgba(250,250,250,0) 0%, #fafafa 92%)' }} />
                <div className="absolute bottom-16 md:bottom-24 left-0 right-0 px-6 md:px-12">
                  <div className="w-full max-w-6xl mx-auto">
                    <div className="flex items-center gap-3 mb-3">
                      <span className="text-xs md:text-sm font-bold text-white/85 uppercase tracking-widest">{selectedProject.category}</span>
                      <span className="text-xs md:text-sm text-white/70">{selectedProject.year}</span>
                    </div>
                    <h1 className="text-3xl md:text-6xl font-bold mb-4 tracking-tight leading-tight text-white drop-shadow-md max-w-3xl">{selectedProject.title}</h1>
                  </div>
                </div>
              </div>
            ) : (
