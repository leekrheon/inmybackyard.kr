...
function ArticlePage({ isAdmin, onOpenEditor }: { isAdmin: boolean; onOpenEditor: (a: Article | null) => void }) {
  ...
  return (
    ...
    >
      {/* 하단 중앙 버튼 그룹 */}
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40 flex items-center gap-1 p-1.5"
        style={{
          borderRadius: 9999,
        ...
        <button
          ...
          className="article-view-btn flex items-center px-4 py-2 rounded-full transition-all duration-300 hover:scale-105 active:scale-95 hover:bg-black group"
...
export default function App() {
  ...
  const returnToSite = useCallback((tab: 'projects' | 'press' | 'contact' | 'article' = 'projects') => {
    // intro는 절대 재활성화하지 않음
    const s = stateRef.current;
    s.isIntroActive = false;
    s.currentIntroProgress = 100;
  ...
  return (
    <>
      <style>{`
        ...
        .article-view-img { filter: brightness(0); transition: filter 0.2s ease; }
        .article-view-btn:hover .article-view-img { filter: brightness(0) invert(1); }
        #intro-progress-bar {
          position: fixed;
          top: 0; left: 0;
          height: 3px;
