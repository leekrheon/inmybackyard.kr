...
// URL에서 OG 메타데이터 가져오기
async function fetchOGData(url: string): Promise<{ title: string; description: string; image: string; source: string } | null> {
  try {
...
function PressAdminSection({ onClose }: { onClose: () => void }) {
  ...
  const handleAdd = async () => {
    ...
    const newItem: Partial<PressItem> = {
      ...
      image_url: og?.image ?? '',
      source_url: url.trim(),
      source_name: og?.source ?? new URL(url.trim()).hostname.replace('www.', ''),
      category,
    ...
    <div className="fixed inset-0 z-[500] bg-white overflow-y-auto">
      ...
      <div className="max-w-4xl mx-auto px-8 py-8">
        ...
        <div className="bg-gray-50 rounded-2xl p-6 mb-8">
          ...
          <p className="text-xs text-gray-400 mt-2">URL 입력 시 제목·설명·이미지를 자동으로 가져옵니다.</p>
        </div>
...
function PressPage({ isAdmin }: { isAdmin: boolean }) {
  ...
  return (
    <>
      ...
      <div className="relative w-full min-h-screen bg-white text-black font-sans" style={{ scrollbarWidth: 'none' }}>
        <style>{`
        ...
        {/* 상단 */}
        <div className="flex flex-col md:flex-row w-full px-6 md:px-12 pt-10 pb-14 gap-10">
          {/* 왼쪽: 소개 */}
          <div className="w-full md:w-1/2">
            <p className="text-2xl md:text-[2rem] leading-tight font-normal mb-6 tracking-tight"
              style={{ fontFamily: 'Pretendard, -apple-system, sans-serif' }}>
              IMBY의 이야기가 담긴 뉴스와 공지를 확인하세요. 브랜드 문화를 만드는 인사이트 에이전시 IMBY의 활동을 소개합니다.
            </p>
          ...
          {/* 오른쪽: 슬라이더 */}
          <div className={`w-full md:w-1/2 transition-opacity duration-300 ${hoveredItem ? 'opacity-0' : 'opacity-100'}`}>
            {loading ? (
              <div className="w-full aspect-video bg-gray-100 animate-pulse rounded" />
            ) : sliderItems.length > 0 ? (
              <>
                <div className="w-full aspect-video bg-gray-100 overflow-hidden relative cursor-pointer"
                  onClick={() => setSelected(sliderItems[sliderIndex])}>
                  {sliderItems[sliderIndex].image_url
                    ? <img src={sliderItems[sliderIndex].image_url} alt={sliderItems[sliderIndex].title}
                ...
                <div className="flex justify-between items-start text-sm mt-2">
                  <div className="flex items-center gap-3">
                    <span className="text-gray-400">{sliderIndex + 1} / {sliderItems.length}</span>
                    <span className="flex gap-2">
                      ...
                      <button onClick={() => setSliderIndex(i => (i + 1) % sliderItems.length)}
                        className="hover:opacity-50 cursor-pointer rotate-180">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
                      </button>
                    </span>
                  </div>
                  <div className="text-right max-w-[60%]">
                    <p className="font-medium leading-snug line-clamp-2">{sliderItems[sliderIndex].title}</p>
                    <button onClick={() => setSelected(sliderItems[sliderIndex])}
        ...
        <div className="w-full px-6 md:px-12 pb-24">
          {/* 헤더 */}
          <div className="flex border-b border-black pb-2 text-[11px] text-gray-400 uppercase tracking-widest mb-1">
          ...
          {loading ? (
            ...
            <div className="flex flex-col">
              {items.map(item => (
                ...
                >
                  <div className="w-[50%] pr-4 truncate group-hover:underline decoration-1 underline-offset-4 font-medium text-sm">{item.title}</div>
                  <div className="w-[20%] pr-4 text-sm text-gray-500">{item.category === 'news' ? 'Press' : 'Notice'}</div>
                  <div className="w-[20%] pr-4 text-sm text-gray-400 truncate">{item.source_name}</div>
        ...
        {hoveredItem?.image_url && (
          <div className="fixed top-0 right-0 w-1/2 h-screen z-40 pointer-events-none hidden md:block"
            style={{ backgroundImage: `url(${hoveredItem.image_url})`, backgroundSize: 'cover', backgroundPosition: 'center', transition: 'opacity 0.3s' }} />
        )}
      </div>
    </>
  );
  ...
  return (
    <div className="fixed inset-0 z-[600] bg-white overflow-y-auto" style={{ animation: 'fadeIn 0.4s ease' }}>
      <button onClick={onClose}
        className="fixed z-[310] flex items-center justify-center w-9 h-9 rounded-full bg-black/80 hover:bg-black text-white shadow-lg transition-all cursor-pointer hover:scale-105"
        style={{ top: '2.875rem', right: '1.8rem', transform: 'translateY(-50%)' }}>
...
export default function App() {
  ...
  const handleLogout = async () => { await supabase.auth.signOut(); setUser(null); returnToSite('projects'); };
  ...
  return (
    <>
      ...
      {/* Press Modal */}
      <div className={`fullscreen-modal fixed inset-0 z-[150] bg-white transition-transform duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] overflow-y-auto ${activeTab === 'press' ? 'translate-y-0' : 'translate-y-full'}`}>
