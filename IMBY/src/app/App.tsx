...
function sanitizeProjectHtml(html: string): string {
  return DOMPurify.sanitize(html ?? '', {
    ALLOWED_TAGS: ['p', 'br', 'b', 'strong', 'i', 'em', 'u', 's', 'span', 'div', 'ul', 'ol', 'li', 'a', 'img', 'video', 'iframe', 'figure', 'figcaption'],
...
function buildEmbedBlockHtml(url: string): string {
  ...
  if (embedSrc) {
    return `<div class="rte-block" contenteditable="false" draggable="true"><div style="position:relative;width:100%;padding-top:56.25%;border-radius:12px;overflow:hidden;background:#000;"><iframe src="${embedSrc}" style="position:absolute;inset:0;width:100%;height:100%;border:0;" sandbox="allow-scripts allow-same-origin allow-presentation" allow="accelerometer; encrypted-media; picture-in-picture" referrerpolicy="no-referrer" loading="lazy"></iframe></div></div><p><br></p>`;
  }
  let hostname = url;
...
function ProjectHero({ project }: { project: Project }) {
  const isLight = useImageBrightness(project.img);
  const dark = isLight !== true; // 알 수 없거나(null) 어두우면 기본값(검은 그라데이션+흰 글씨) 사용
  const readabilityGradient = dark
    ? 'linear-gradient(to top, rgba(0,0,0,0.75) 0%, rgba(0,0,0,0.3) 38%, rgba(0,0,0,0) 62%)'
    : 'linear-gradient(to top, rgba(255,255,255,0.88) 0%, rgba(255,255,255,0.4) 38%, rgba(255,255,255,0) 62%)';
  const fadeToBg = 'linear-gradient(to bottom, rgba(250,250,250,0) 0%, #fafafa 92%)';
  const titleColorClass = dark ? 'text-white drop-shadow-md' : 'text-gray-900';
  const metaColorClass = dark ? 'text-white/85' : 'text-gray-700';
  const metaColorClass2 = dark ? 'text-white/70' : 'text-gray-500';

  if (!project.img) {
    return (
      <div className="w-full max-w-6xl mx-auto pt-20 px-6 md:pt-12 md:px-12">
        <div className="mb-4 flex items-center gap-3">
          <span className="text-sm font-bold text-gray-400 uppercase tracking-widest">{project.category}</span>
          <span className="text-sm text-gray-400">{project.year}</span>
        </div>
        <h1 className="text-4xl md:text-6xl font-bold mb-6 tracking-tight leading-tight">{project.title}</h1>
      </div>
    );
  }

  return (
    <div className="relative w-full overflow-hidden project-hero" style={{ marginTop: '-6rem', height: '58vh', minHeight: 380 }}>
      <img src={project.img} alt={project.title}
        style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center' }} />
      {/* 제목 가독성을 위한 그라데이션 (이미지 밝기에 따라 자동 반전) */}
      <div className="absolute inset-0" style={{ background: readabilityGradient }} />
      {/* 하단부를 페이지 배경색으로 자연스럽게 페이드시켜 본문으로 이어지게 함 */}
      <div className="absolute bottom-0 left-0 right-0" style={{ height: '35%', background: fadeToBg }} />
      <div className="absolute bottom-[15%] left-0 right-0 px-6 md:px-12">
        <div className="w-full max-w-6xl mx-auto">
          <div className="flex items-center gap-3 mb-3">
            <span className={`text-xs md:text-sm font-bold uppercase tracking-widest ${metaColorClass}`}>{project.category}</span>
            <span className={`text-xs md:text-sm ${metaColorClass2}`}>{project.year}</span>
          </div>
          <h1 className={`text-3xl md:text-6xl font-bold mb-4 tracking-tight leading-tight max-w-3xl ${titleColorClass}`}>{project.title}</h1>
        </div>
      </div>
      <style>{`
        @media (min-width: 768px) {
          .project-hero { height: 74vh !important; }
        }
      `}</style>
    </div>
  );
...
function InlineRichEditor({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  const handleAddEmbed = () => {
    const url = prompt('임베드할 URL을 입력하세요 (유튜브·비메오는 재생창으로, 그 외 링크는 미리보기 카드로 삽입돼요)');
    if (!url) return;
    try { new URL(url); } catch { alert('올바른 URL 형식이 아닙니다.'); return; }
    insertHtmlAtCursor(buildEmbedBlockHtml(url));
  };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-1 px-4 py-2 border-b border-gray-200 bg-gray-50 flex-wrap sticky top-0 z-10">
        <select value={fontSize} onChange={e => applyFontSize(e.target.value)}
          className="text-xs border border-gray-200 rounded px-2 py-1 bg-white mr-1" style={{ fontFamily: 'inherit' }}>
          {FONT_SIZES.map(s => <option key={s} value={s}>{s}</option>)}
        </select>

        <div className="w-px h-5 bg-gray-200 mx-1" />

        <button type="button" onClick={() => exec('bold')} className="w-8 h-8 flex items-center justify-center rounded hover:bg-gray-200 transition-colors text-sm font-bold">B</button>
        <button type="button" onClick={() => exec('italic')} className="w-8 h-8 flex items-center justify-center rounded hover:bg-gray-200 transition-colors text-sm italic font-serif">I</button>
        <button type="button" onClick={() => exec('underline')} className="w-8 h-8 flex items-center justify-center rounded hover:bg-gray-200 transition-colors text-sm underline">U</button>
        <button type="button" onClick={() => exec('strikeThrough')} className="w-8 h-8 flex items-center justify-center rounded hover:bg-gray-200 transition-colors text-sm line-through">S</button>

        <div className="w-px h-5 bg-gray-200 mx-1" />

        <div className="relative">
          <button type="button" onClick={() => setShowHighlights(v => !v)}
            className="flex items-center gap-1 px-2 h-8 rounded hover:bg-gray-200 transition-colors text-xs font-medium">
            <span style={{ background: 'linear-gradient(90deg,#FFF176,#A5F3A5,#93C5FD)', borderRadius: 2, padding: '0 6px', fontSize: 11 }}>형광펜</span>
            <span className="text-gray-400">▾</span>
          </button>
        ...
        <button type="button" onClick={() => exec('insertUnorderedList')} className="w-8 h-8 flex items-center justify-center rounded hover:bg-gray-200 transition-colors text-xs" title="목록">≡</button>
        <button type="button" onClick={() => exec('removeFormat')} className="text-xs text-gray-400 hover:text-black px-2 h-8 rounded hover:bg-gray-200 transition-colors">서식 제거</button>

        <div className="w-px h-5 bg-gray-200 mx-1" />

        {/* 사진/동영상/링크 삽입 — 커서 위치에 바로 삽입되고, 이후 드래그로 위치 이동 가능 */}
        <label className="flex items-center gap-1 px-2 h-8 rounded hover:bg-gray-200 transition-colors text-xs font-medium cursor-pointer"
          onMouseDown={saveSelection}>
          <input type="file" accept="image/*" multiple className="hidden" onChange={handleAddImages} />
          {imgUploading ? '업로드 중…' : '🖼 사진'}
        </label>
        <label className="flex items-center gap-1 px-2 h-8 rounded hover:bg-gray-200 transition-colors text-xs font-medium cursor-pointer"
          onMouseDown={saveSelection}>
          <input type="file" accept="video/mp4,video/webm,video/quicktime,video/ogg" className="hidden" onChange={handleAddVideo} />
          {vidUploading ? '업로드 중…' : '🎬 동영상'}
        </label>
        <button type="button" onMouseDown={saveSelection} onClick={handleAddEmbed}
          className="flex items-center gap-1 px-2 h-8 rounded hover:bg-gray-200 transition-colors text-xs font-medium">🔗 링크 임베드</button>

        <span className="ml-auto text-[11px] text-gray-400 hidden md:inline">사진을 클릭해 선택한 뒤 드래그하면 위치를 옮길 수 있어요</span>
      </div>
      ...
      <div
        ...
        className="flex-1 overflow-y-auto px-8 py-6 focus:outline-none text-gray-800 leading-relaxed rte-editor"
        style={{ minHeight: '300px', fontFamily: 'inherit', fontSize: '16px', lineHeight: 1.8 }}
        data-placeholder="프로젝트 내용을 작성하세요. 툴바의 사진/동영상/링크 버튼으로 원하는 위치에 바로 삽입할 수 있어요."
      />
      <style>{`
        [data-placeholder]:empty:before {
          content: attr(data-placeholder);
        ...
        .rte-block:hover { outline: 2px dashed #0FCD60; }
                ...
                <tbody>
                  {filtered.map((p, i) => (
                    <tr key={p.id}
                      ...
                      style={{ borderBottom: i < filtered.length - 1 ? '1px solid #f9fafb' : 'none', opacity: p.hidden ? 0.5 : 1 }}>
                      {/* Drag handle */}
                      <td className="pl-3 pr-0 py-3.5 text-gray-300 cursor-grab" title="드래그해서 순서 변경">
                      ...
                      <td className="px-4 py-3.5 max-w-xs">
                        <div className="flex items-center gap-2">
                          ...
                          {p.hidden && (
                            <span className="text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full bg-gray-200 text-gray-500 shrink-0">숨김</span>
                          )}
                        </div>
                      ...
                      <td className="px-6 py-3.5">
                        <div className="flex items-center gap-1.5 justify-end">
                          {confirmId === p.id ? (
                            ...
                            <>
                              <button onClick={() => onToggleHidden(p)}
...
async function dbUpsertArticle(a: Partial<Article> & { id?: number }): Promise<Article> {
  const { data, error } = await supabase.from('articles').upsert(a).select().single();
  if (error) { console.error(error); throw new Error(error.message || '저장에 실패했습니다.'); }
  return data as Article;
}
  ...
  return (
    <>
      <style>{`
        ...
        .gallery-img { height: 55vh; width: 36vw; max-width: 36vw; object-fit: cover; object-position: center; transform-origin: center; transition: transform 0.8s cubic-bezier(0.16, 1, 0.3, 1); }
          ...
          {loading ? (
            ...
            <>
              {filteredProjects.map((proj, index) => (
                ...
                >
                  <div className="gallery-img-container relative" style={proj.hidden ? { opacity: 0.4 } : undefined}>
                    ...
                    {isAdmin && (
                      <button className="edit-btn absolute top-2 right-2" onClick={e => { e.stopPropagation(); openEditor(proj); }}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg> 편집</button>
                    )}
                    {proj.hidden && isAdmin && (
                      <span className="absolute top-2 left-2 bg-black/70 text-white text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-full">숨김</span>
                    )}
                  </div>
                  <div className="gallery-info">
                    ...
                    <h3 className="font-medium text-xl md:text-2xl leading-snug">{proj.title}</h3>
      ...
      <div aria-hidden={selectedProjectIndex === null} className={`fullscreen-modal fixed inset-0 z-[200] bg-[#fafafa] transition-transform duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] overflow-y-auto pt-24 ${selectedProjectIndex !== null ? 'translate-y-0' : 'translate-y-full'}`}>
        ...
        {selectedProject && (
          ...
          <div className="w-full max-w-6xl mx-auto pb-12 px-6 md:pb-12 md:px-12">
