...
export default function App() {
  ...
  useEffect(() => {
    ...
    function renderIntroSequence() {
      ...
      // ment3: progress 56~62 등장, 72~78 퇴장
      ...
      // boy.png — progress에 따라 위에서 내려오고 역스크롤 시 올라감
      const boyEl = document.getElementById('boy-img');
      if (boyEl) {
        const boyP = mapRange(s.currentIntroProgress, 0, 10, 0, 1);
        const boyY = -220 * (1 - boyP);
        boyEl.style.transform = `translate(-50%, ${boyY}px)`;
        boyEl.style.opacity = String(Math.min(boyP * 2, 1) * mapRange(s.currentIntroProgress, 11, 15, 1, 0));
      }
        ...
        <div ref={step2ContainerRef} className="intro-layer" style={{ pointerEvents: 'none' }}>
          ...
          <img id="slide1-info" src="/info1.png" alt="info1"
            style={{ position: 'absolute', bottom: 0, left: '50%', transform: 'translateX(-50%)',
              height: 'clamp(280px, 55vh, 650px)', width: 'auto', objectFit: 'contain',
              opacity: 0, pointerEvents: 'none' }} />
          {/* ment1 — 상단 중앙 */}
          <img id="slide1-ment" src="/ment1.png" alt="ment1"
            style={{ position: 'absolute', top: '6vh', left: '50%', transform: 'translateX(-50%)',
              width: 'clamp(320px, 72vw, 900px)', objectFit: 'contain',
              filter: 'brightness(0)', opacity: 0, pointerEvents: 'none' }} />

          {/* ── slide 2: info2 오른쪽 먼저, ment2 왼쪽 연이어 ── */}
          {/* info2 — 오른쪽, 전체 높이 */}
          <div id="slide2-info" style={{ position: 'absolute', top: 0, right: 0, width: '50%', height: '100%',
          ...
          <div id="slide2-ment" style={{ position: 'absolute', top: 0, left: 0, width: '50%', height: '100%',
            ...
            <img ref={typo2Ref} src="/ment2.png" alt="ment2"
              style={{ width: '100%', objectFit: 'contain', filter: 'brightness(0)' }} />
          </div>
        </div>

        <div ref={step4ContainerRef} className="intro-layer" style={{ pointerEvents: 'none' }}>
          {/* ── slide 3: ment3 상단 중앙 (info3 없음) ── */}
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start' }}>
            <img ref={typo3Ref} src="/ment3.png" alt="ment3"
              style={{ width: 'clamp(320px, 72vw, 900px)', objectFit: 'contain', display: 'block', filter: 'brightness(0)', marginTop: '6vh', opacity: 0 }} />
          </div>
        </div>
