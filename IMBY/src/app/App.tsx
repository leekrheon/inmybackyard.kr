...
// ─── 본문 블록 에디터 (네이버 블로그 스타일) ──────────────────────────────────
// content 컬럼에는 블록 배열을 JSON 문자열로 저장한다.
// 과거에 저장된 순수 HTML 문자열(레거시)도 자동으로 텍스트 블록 1개로 변환해 호환한다.
interface ContentBlock {
  id: string;
  type: 'text' | 'image' | 'video' | 'embed';
  html?: string;      // text
  url?: string;       // image / video / embed
  caption?: string;   // image / video / embed
}

function parseContentBlocks(raw: string): ContentBlock[] {
  if (!raw) return [{ id: 'b0', type: 'text', html: '' }];
  try {
  ...
  return [{ id: 'legacy', type: 'text', html: raw }];
}
...
// 임의 URL을 안전하게 렌더링: 영상 플랫폼은 sandbox iframe, 그 외는 링크 미리보기 카드.
function EmbedBlockView({ url }: { url: string }) {
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer nofollow"
      className="block rounded-xl overflow-hidden border border-gray-200 hover:shadow-md transition-shadow bg-gray-50">
      <img
        src={`https://api.microlink.io/?url=${encodeURIComponent(url)}&screenshot=true&meta=false&embed=screenshot.url`}
        alt="link preview" className="w-full h-auto object-cover" loading="lazy"
        onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }}
      />
      <div className="px-4 py-3 flex items-center gap-2 min-w-0">
        <img src={`https://www.google.com/s2/favicons?domain=${hostname}&sz=32`} alt="" className="w-4 h-4 rounded shrink-0" />
...
// 저장된 블록을 실제 화면에 렌더링 (공개 페이지용, 텍스트는 XSS 방지를 위해 sanitize)
function ContentBlocksRenderer({ content }: { content: string }) {
      ...
      {blocks.map(b => {
        if (b.type === 'text') {
          const clean = DOMPurify.sanitize(b.html ?? '', {
            ALLOWED_TAGS: ['p', 'br', 'b', 'strong', 'i', 'em', 'u', 's', 'span', 'div', 'ul', 'ol', 'li', 'a'],
            ALLOWED_ATTR: ['style', 'href', 'target', 'rel'],
          });
          if (!clean.trim()) return null;
          return <div key={b.id} dangerouslySetInnerHTML={{ __html: clean }} />;
        }
        if (b.type === 'image' && b.url) {
          return (
            <figure key={b.id}>
              <img src={b.url} alt={b.caption ?? ''} className="w-full h-auto rounded-xl" loading="lazy" />
              {b.caption && <figcaption className="mt-2 text-sm text-gray-400 text-center">{b.caption}</figcaption>}
            </figure>
          );
        }
        if (b.type === 'video' && b.url) {
          return (
            <figure key={b.id}>
              <video src={b.url} controls preload="metadata" className="w-full h-auto rounded-xl bg-black" />
              {b.caption && <figcaption className="mt-2 text-sm text-gray-400 text-center">{b.caption}</figcaption>}
            </figure>
          );
        }
        if (b.type === 'embed' && b.url) {
          return <EmbedBlockView key={b.id} url={b.url} />;
        }
        return null;
      })}
    </div>
...
// ═══════════════════════════════════════════════════════════════════════════════
// Block Editor — 네이버 블로그 스타일: 텍스트/사진/동영상/링크임베드 블록을
// 자유롭게 추가하고 드래그로 순서를 바꿀 수 있는 에디터
// ═══════════════════════════════════════════════════════════════════════════════
function TextBlockEditor({ html, onChange }: { html: string; onChange: (v: string) => void }) {
  ...
  const exec = (cmd: string, val?: string) => { ref.current?.focus(); document.execCommand(cmd, false, val); handleInput(); };
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden">
      <div className="flex items-center gap-1 px-2 py-1.5 bg-gray-50 border-b border-gray-100">
        <button type="button" onMouseDown={e => e.preventDefault()} onClick={() => exec('bold')} className="w-7 h-7 text-xs font-bold rounded hover:bg-gray-200">B</button>
        <button type="button" onMouseDown={e => e.preventDefault()} onClick={() => exec('italic')} className="w-7 h-7 text-xs italic rounded hover:bg-gray-200">I</button>
        <button type="button" onMouseDown={e => e.preventDefault()} onClick={() => exec('underline')} className="w-7 h-7 text-xs underline rounded hover:bg-gray-200">U</button>
        <button type="button" onMouseDown={e => e.preventDefault()} onClick={() => exec('hiliteColor', '#FFF176')} className="w-7 h-7 text-xs rounded hover:bg-gray-200">형광</button>
        <button type="button" onMouseDown={e => e.preventDefault()} onClick={() => exec('insertUnorderedList')} className="w-7 h-7 text-xs rounded hover:bg-gray-200">≡</button>
        <button type="button" onMouseDown={e => e.preventDefault()} onClick={() => exec('removeFormat')} className="ml-auto text-[11px] text-gray-400 hover:text-black px-2 h-7 rounded hover:bg-gray-200">서식 제거</button>
      </div>
      <div ref={ref} contentEditable suppressContentEditableWarning onInput={handleInput}
        className="px-4 py-3 min-h-[80px] text-gray-800 leading-relaxed focus:outline-none"
        style={{ fontSize: '16px', lineHeight: 1.8 }} data-placeholder="내용을 입력하세요..." />
    </div>
  );
}
...
function BlockEditor({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  const commit = (next: ContentBlock[]) => { setBlocks(next); onChange(serializeContentBlocks(next)); };
  ...
  const addBlock = (b: ContentBlock) => commit([...blocks, b]);
  ...
  const handleAddImages = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const f of files) {
        ...
        addBlock({ id: newBlockId(), type: 'image', url, caption: '' });
      }
  ...
  const handleAddVideo = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      addBlock({ id: newBlockId(), type: 'video', url, caption: '' });
  ...
  const handleAddEmbed = () => {
    const url = prompt('임베드할 URL을 입력하세요 (예: 유튜브, 인스타그램, 웹사이트 링크 등)');
    if (!url) return;
    try { new URL(url); } catch { alert('올바른 URL 형식이 아닙니다.'); return; }
    addBlock({ id: newBlockId(), type: 'embed', url });
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto px-8 py-6 flex flex-col gap-3">
      {blocks.map((b, i) => (
        ...
        >
          {/* 드래그 핸들 + 컨트롤 */}
          <div className="flex flex-col items-center gap-1 pt-2 shrink-0 cursor-grab select-none text-gray-300 hover:text-gray-500" title="드래그해서 순서 변경">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="9" cy="6" r="1.6"/><circle cx="15" cy="6" r="1.6"/><circle cx="9" cy="12" r="1.6"/><circle cx="15" cy="12" r="1.6"/><circle cx="9" cy="18" r="1.6"/><circle cx="15" cy="18" r="1.6"/></svg>
            <button type="button" onClick={() => moveBlock(i, i - 1)} className="hover:text-black">▲</button>
            <button type="button" onClick={() => moveBlock(i, i + 1)} className="hover:text-black">▼</button>
            <button type="button" onClick={() => removeBlock(b.id)} className="hover:text-red-500">✕</button>
          </div>

          <div className="flex-1 min-w-0">
            {b.type === 'text' && (
              <TextBlockEditor html={b.html ?? ''} onChange={v => updateBlock(b.id, { html: v })} />
            )}
            {b.type === 'image' && b.url && (
              <div>
                <img src={b.url} alt="" className="w-full h-auto rounded-xl border border-gray-100" />
                <input type="text" value={b.caption ?? ''} onChange={e => updateBlock(b.id, { caption: e.target.value })}
                  placeholder="사진 설명 (선택)" className="w-full mt-2 border border-gray-200 rounded-lg px-3 py-1.5 text-xs" />
              </div>
            )}
            {b.type === 'video' && b.url && (
              <div>
                <video src={b.url} controls className="w-full h-auto rounded-xl bg-black" />
                <input type="text" value={b.caption ?? ''} onChange={e => updateBlock(b.id, { caption: e.target.value })}
                  placeholder="동영상 설명 (선택)" className="w-full mt-2 border border-gray-200 rounded-lg px-3 py-1.5 text-xs" />
              </div>
            )}
            {b.type === 'embed' && b.url && (
              <div>
                <EmbedBlockView url={b.url} />
                <input type="text" value={b.url} onChange={e => updateBlock(b.id, { url: e.target.value })}
                  className="w-full mt-2 border border-gray-200 rounded-lg px-3 py-1.5 text-xs" />
              </div>
            )}
          </div>
        </div>
      ...
      {/* 블록 추가 툴바 */}
      <div className="sticky bottom-0 mt-2 flex items-center gap-2 flex-wrap bg-white/95 backdrop-blur border-t border-gray-100 pt-3 pb-1">
        <button type="button" onClick={() => addBlock({ id: newBlockId(), type: 'text', html: '' })}
          className="flex items-center gap-1.5 text-xs font-medium px-3 py-2 rounded-lg border border-gray-200 hover:border-[#0FCD60] hover:text-[#0FCD60] transition-colors">＋ 텍스트</button>
        <label className="flex items-center gap-1.5 text-xs font-medium px-3 py-2 rounded-lg border border-gray-200 hover:border-[#0FCD60] hover:text-[#0FCD60] transition-colors cursor-pointer">
          <input type="file" accept="image/*" multiple className="hidden" onChange={handleAddImages} />
          {imgUploading ? '업로드 중…' : '＋ 사진 (원본화질)'}
        </label>
        <label className="flex items-center gap-1.5 text-xs font-medium px-3 py-2 rounded-lg border border-gray-200 hover:border-[#0FCD60] hover:text-[#0FCD60] transition-colors cursor-pointer">
          <input type="file" accept="video/mp4,video/webm,video/quicktime,video/ogg" className="hidden" onChange={handleAddVideo} />
          {vidUploading ? '업로드 중…' : '＋ 동영상 (원본화질)'}
        </label>
        <button type="button" onClick={handleAddEmbed}
          className="flex items-center gap-1.5 text-xs font-medium px-3 py-2 rounded-lg border border-gray-200 hover:border-[#0FCD60] hover:text-[#0FCD60] transition-colors">＋ 링크 임베드</button>
      </div>
    </div>
  );
}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            <BlockEditor value={form.content} onChange={v => set('content', v)} />
          </div>
        </div>
      </div>
    </div>
    ...
    <>
      ...
      <div className="relative w-full min-h-screen bg-white text-black" style={{ fontFamily: "Pretendard, -apple-system, BlinkMacSystemFont, sans-serif", scrollbarWidth: 'none' as const }}>
        ...
        <div className="w-full px-6 md:px-12 pb-24 mt-4">
          ...
          <div className="hidden md:flex border-b border-black pb-2 text-[11px] font-semibold text-black uppercase tracking-widest mb-1">
            ...
            <div className="flex flex-col">
              {items.map(item => (
                <React.Fragment key={item.id}>
                  ...
                  <div
                    className="press-row flex flex-col gap-1.5 py-3.5 md:flex-row md:items-center md:gap-0 cursor-pointer transition-colors duration-150 group"
                    onClick={() => setExpandedId(expandedId === item.id ? null : item.id)}
                  >
                    <div className="w-full md:w-[50%] pr-0 md:pr-4 md:truncate group-hover:underline decoration-1 underline-offset-4 font-bold text-sm text-black leading-snug">{item.title}</div>
                    ...
                    <div className="hidden md:block w-[20%] pr-4 text-sm font-semibold text-black">{item.category === 'news' ? 'Press' : 'Notice'}</div>
                    <div className="hidden md:block w-[20%] pr-4 text-sm font-semibold text-black truncate">{item.source_name}</div>
                    <div className="hidden md:block w-[10%] text-right text-sm font-semibold text-black">
...
function ArticleDetailPage({ article, onClose }: { article: Article; onClose: () => void }) {
  useEffect(() => {
    ...
    trackPageView(`/article/${article.slug}`, `${article.title} — Article | IMBY`);
            ...
            {article.content
              ? <div className="project-content text-gray-800 text-lg leading-relaxed font-light"
                  dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(article.content, {
                    ALLOWED_TAGS: ['p','br','b','strong','i','em','u','s','span','div','ul','ol','li','a','img'],
    ...
    <>
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          /* ── Glass nav: 하단 고정 ── */
          /* 데스크탑과 같은 PNG 워드마크를 다시 사용 — 단, 이전에 화면 밖으로 넘쳤던(overflow)
             원인이 "높이(height)를 22px 이상으로 키운 것"이었으므로, 지금 텍스트 버전이 차지하던
             폭(약 366px)과 비슷하게 나오는 높이(17px)로 맞춰서 잘림 없이 데스크탑과 동일한 폰트로 표시.
             활성 탭도 흰색 반전(invert) 대신 검정 유지 — iOS Safari에서 backdrop-filter 부모 안 자식
             filter(invert)가 안 보이던 버그를 원천적으로 피하기 위함 (초록 배경 위 검정 글자도 대비 충분). */
          .glass-nav {
            top: auto !important;
          ...
          .glass-nav button { padding: 0.5rem 0.7rem !important; }
          .nav-label {
          ...
          .glass-nav button.active .nav-label,
          .glass-nav button:hover .nav-label,
          .glass-nav button .nav-label {
            filter: brightness(0) !important;
          }

          /* ── Projects: 필터 한 줄 (logo 아래) ── */
          #sub-menu {
            /* top:3.8rem처럼 오프셋으로 띄우는 대신, 배경 자체를 화면 맨 위(0)부터 시작해서
               로고 밑~바 사이 구간까지 통째로 덮음 → 뒤 이미지가 절대 안 비치도록 원천 차단.
               (기존엔 gallery-wrapper의 padding-top 수치에 의존해서, 조금만 어긋나도 이미지가
               로고와 바 사이 틈으로 비쳐 보였음) */
            top: 0 !important;
            height: 7rem !important;
            padding: 4.7rem 1rem 0.9rem 1rem !important;
            ...
            /* 반투명 유리 느낌 대신 완전 불투명 배경 + 그림자로 "떠있는" 느낌 제거하고 확실히 구분 */
            background: #ffffff !important;
            box-shadow: 0 2px 14px rgba(0,0,0,0.08) !important;
            /* 다른 fullscreen-modal(z-[200])과 같은 값이라 전환 중 화면이 이 바 위로 지나가던 문제 → 항상 최상단으로 */
          ...
          /* 모바일에서는 좌측 삼각형 표시 제거 (가로 배치라 불필요) */
          #sub-menu a::before { display: none !important; }
          .filter-label { height: 16px !important; }

          #gallery-wrapper {
            align-items: flex-start !important;
            /* 필터 바(top 3.8rem + 자체 높이)를 확실히 벗어난 지점부터 갤러리 시작 —
               기존 12vw는 화면폭에 따라 바 높이보다 작아져서 이미지가 바 뒤로 겹쳐 보이던 문제 */
          ...
          /* 강제 4:3 비율 + cover가 원본 이미지 비율을 잘라내던 문제 →
             데스크탑처럼 원본 비율 그대로, 폭만 맞추고 높이는 자동 계산 (잘림 없음) */
          .gallery-img { height: auto !important; width: 82vw !important; max-width: 82vw !important; }
        ...
        @media (max-width: 768px) {
          /* 모바일: 페이지 전환이 단순히 위로 슬라이드하는 것처럼만 보이지 않도록,
             카드/시트 형태로 구분감(둥근 상단 + 그림자)을 줘서 화면끼리 경계가 뚜렷하게 보이도록 함 */
          .fullscreen-modal {
      ...
      <div ref={glassNavRef} id="glass-nav" className="glass-nav no-select">
        <button data-label="PROJECT" className={activeTab === 'projects' ? 'active' : ''} onClick={() => handleTabClick('projects')}>
          <img src="/nav-project.png" alt="PROJECT" className="nav-label" />
        </button>
        <button data-label="PRESS" className={activeTab === 'press' ? 'active' : ''} onClick={() => handleTabClick('press')}>
          <img src="/nav-press.png" alt="PRESS" className="nav-label" />
        </button>
        <button data-label="ARTICLE" className={activeTab === 'article' ? 'active' : ''} onClick={() => handleTabClick('article')}>
          <img src="/nav-article.png" alt="ARTICLE" className="nav-label" />
        </button>
        <button data-label="CONTACT" className={activeTab === 'contact' ? 'active' : ''} onClick={() => handleTabClick('contact')}>
                ...
                <div key={proj.id} className="gallery-item group" onClick={() => setSelectedProjectIndex(index)}>
                  ...
                  <div className="gallery-info">
                    ...
                    <div className="relative overflow-hidden">
                      <h3 className="relative z-10 font-medium text-xl md:text-2xl">{proj.title}</h3>
                      {proj.img && (
                        <img src={proj.img} alt="" aria-hidden="true" draggable="false"
                          className="absolute inset-y-0 right-0 z-0 w-1/2 h-full object-cover pointer-events-none select-none"
                          style={{
                    ...
                    <p className="relative z-10 text-gray-500 mt-1 text-sm md:text-base">{proj.description}</p>
                  </div>
                </div>
              ))}
              {isAdmin && (
      ...
      <div aria-hidden={selectedProjectIndex === null} className={`fullscreen-modal fixed inset-0 z-[200] bg-[#fafafa] transition-transform duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] overflow-y-auto pt-24 ${selectedProjectIndex !== null ? 'translate-y-0' : 'translate-y-full'}`}>
        <button onClick={() => { setSelectedProjectIndex(null); setActiveTab('projects'); window.history.pushState({}, '', '/projects'); }} className="fixed top-5 right-5 w-10 h-10 md:top-24 md:right-8 md:w-12 md:h-12 flex items-center justify-center rounded-full bg-white shadow-md hover:bg-gray-100 transition-colors z-10 cursor-pointer">
        ...
        {isAdmin && selectedProject && (
          <button onClick={() => openEditor(selectedProject)} className="fixed top-5 right-16 px-3 py-2 text-xs md:top-24 md:right-24 md:px-4 md:py-3 md:text-sm flex items-center gap-1.5 md:gap-2 rounded-full bg-[#0FCD60] text-white font-bold shadow-lg hover:bg-green-500 transition-colors z-10 cursor-pointer"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg> 편집</button>
        )}
        {selectedProject && (
          <div className="w-full max-w-6xl mx-auto pt-20 pb-12 px-6 md:py-12 md:px-12">
            <div className="mb-4 flex items-center gap-3">
              <span className="text-sm font-bold text-gray-400 uppercase tracking-widest">{selectedProject.category}</span>
              <span className="text-sm text-gray-400">{selectedProject.year}</span>
            </div>
            <div className="mb-16 max-w-3xl project-detail-header">
              ...
              {/* 블록 콘텐츠 (텍스트/사진/동영상/링크임베드), XSS 방지 sanitize 적용 */}
              <div className="project-content text-lg text-gray-800 leading-relaxed font-light">
                <ContentBlocksRenderer content={selectedProject.content} />
              </div>
