...
function ProjectHero({ project }: { project: Project }) {
  ...
  if (!project.img) {
    return (
      <div className="w-full max-w-[1400px] mx-auto pt-20 px-6 md:pt-12 md:px-12">
        <div className="mb-4 flex items-center gap-3">
          <span className="text-sm font-bold text-gray-400 uppercase tracking-widest">{project.category}</span>
          <span className="text-sm text-gray-400">{project.year}</span>
        </div>
        <h1 className="text-4xl md:text-6xl font-bold mb-3 tracking-tight leading-tight">{project.title}</h1>
        {project.description && (
          <p className="text-lg md:text-2xl text-gray-500 font-medium leading-snug mb-6 max-w-4xl">{project.description}</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="relative w-full overflow-hidden project-hero" style={{ marginTop: '-6rem', height: 'clamp(420px, 62vh, 720px)' }}>
      ...
      <div className="absolute bottom-0 left-0 right-0" style={{ height: '22%', background: fadeToBg }} />
      {/* 본문 컨테이너(max-w-[1400px] + px-12)와 정확히 같은 방식으로 좌측 정렬 —
          기존엔 바깥에 px가 있고 안쪽에서 다시 중앙정렬돼서 본문/사진보다 48px 더 왼쪽에 붙었음 */}
      <div className="absolute bottom-[10%] left-0 right-0">
        <div className="w-full max-w-[1400px] mx-auto px-6 md:px-12">
          ...
          <h1 className={`text-3xl md:text-5xl lg:text-6xl font-bold mb-3 tracking-tight leading-tight w-full project-hero-title ${titleColorClass}`}>{project.title}</h1>
          {/* 부제목(description) — 본문이 아니라 제목 바로 아래에 표시 */}
          {project.description && (
            <p className={`text-base md:text-xl lg:text-2xl font-medium leading-snug max-w-4xl ${metaColorClass}`}>{project.description}</p>
          )}
        </div>
      </div>
      <style>{`
        @media (min-width: 768px) {
          .project-hero { height: clamp(520px, 78vh, 860px) !important; }
          /* 제목은 PC에서 항상 한 줄. 컨테이너(1400px)보다 긴 제목은 폰트가 자동으로 줄어들어
             줄바꿈 없이 한 줄에 들어가도록 함 */
          .project-hero-title {
...
function InlineRichEditor({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Toolbar */}
      <div className="flex items-center gap-1 px-4 py-2 border-b border-gray-100 bg-white flex-wrap sticky top-0 z-10">
        ...
        <button type="button" onClick={() => exec('bold')} className={`${btnBase} w-8 text-sm font-bold`}>B</button>
...
async function fetchOGData(url: string): Promise<{ title: string; description: string; image: string; source: string }> {
  ...
  // 각 제공자가 응답하지 않을 때 무한정 "가져오는 중…"에 머무르지 않도록 요청별 타임아웃을 건다
  const fetchWithTimeout = async (u: string, ms = 7000) => {
  ...
  // 1차: microlink.io
  try {
    const res = await fetchWithTimeout(`https://api.microlink.io/?url=${encodeURIComponent(url)}&timeout=10000&retry=3`);
  ...
  try {
    const res = await fetchWithTimeout(`https://jsonlink.io/api/extract?url=${encodeURIComponent(url)}`);
    const data = await res.json();
    if (data.title) {
      return {
        title: data.title ?? '',
  ...
  try {
    const res = await fetchWithTimeout(`https://opengraph.io/api/1.1/site/${encodeURIComponent(url)}?app_id=unset`);
    const data = await res.json();
  ...
  return (
    <>
      <style>{`
        ...
        /* 컨테이너가 이미지 원본 비율대로 폭을 갖도록(고정 폭 X) — 잘림 방지 */
        .gallery-img-container { overflow: hidden; background-color: #f3f4f6; display: block; width: fit-content; min-width: 12vw; max-width: 70vw; }
        /* 높이만 고정하고 폭은 원본 비율대로 자동 계산 → object-fit: cover로 인한 4:3 강제 크롭 제거.
           세로로 매우 긴 이미지는 max-width 안에서 축소되도록 contain 사용 (잘림 없음) */
        .gallery-img { height: 55vh; width: auto; max-width: 70vw; object-fit: contain; object-position: center; display: block; transform-origin: center; transition: transform 0.8s cubic-bezier(0.16, 1, 0.3, 1); }
        ...
        @media (max-width: 768px) {
          ...
          .gallery-img-container { width: 82vw !important; min-width: 0 !important; max-width: 82vw !important; }
          .gallery-img { height: auto !important; width: 82vw !important; max-width: 82vw !important; object-fit: contain !important; }
        ...
        /* 본문 기본 서체를 에디터와 동일하게 Pretendard로 맞춤 (에디터 화면 = 실제 화면) */
        .project-content, .rte-editor { font-family: Pretendard, 'Pretendard Variable', -apple-system, BlinkMacSystemFont, 'Apple SD Gothic Neo', sans-serif; }
      ...
      <div aria-hidden={selectedProjectIndex === null} className={`fullscreen-modal fixed inset-0 z-[200] bg-[#fafafa] transition-transform duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] overflow-y-auto pt-24 ${selectedProjectIndex !== null ? 'translate-y-0' : 'translate-y-full'}`}>
        ...
        {selectedProject && (
          ...
          <div className="w-full max-w-[1400px] mx-auto pb-12 px-6 md:pb-12 md:px-12">
            ...
            <div className="mb-16 w-full project-detail-header">
              ...
              <div className="project-content text-lg text-gray-800 leading-relaxed font-normal"
