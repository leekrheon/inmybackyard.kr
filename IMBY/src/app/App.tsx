...
function ProjectEditorPage({
  ...
  const handleThumbUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true); set('img', await fileToDataURL(file)); setUploading(false);
  };
...
function AdminDashboard({
  ...
  return (
    <div className="fixed inset-0 z-[400] bg-[#f8f8f8] flex flex-col overflow-hidden">
      {/* Header */}
      <div className="flex items-center gap-4 px-8 py-5 bg-black text-white shrink-0">
        <button onClick={onClose} className="flex items-center gap-2 text-sm text-gray-400 hover:text-white transition-colors">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
          사이트로
        </button>
        <span className="text-gray-600">|</span>
        <span className="text-sm font-bold tracking-widest" style={{ fontFamily: "'League Gothic', sans-serif", fontSize: '1.1rem', letterSpacing: '0.1em' }}>IMBY ADMIN</span>
        <div className="flex-1" />
        <span className="text-xs text-gray-400 hidden md:block">{userEmail}</span>
        <button onClick={onLogout} className="text-xs text-gray-400 hover:text-white transition-colors ml-2">로그아웃</button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-8 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Web', value: projects.filter(p => p.category === 'Web').length },
            { label: 'Campaign', value: projects.filter(p => p.category === 'Campaign').length },
            { label: 'Design / Film', value: projects.filter(p => p.category === 'Design' || p.category === 'Film').length },
          ].map(s => (
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold tracking-tight">프로젝트 관리</h2>
          <button onClick={onNew}
            className="flex items-center gap-2 bg-[#0FCD60] text-white text-sm font-bold px-4 py-2.5 rounded-xl hover:bg-green-500 transition-colors">
            + 새 프로젝트
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          {projects.length === 0 ? (
            <div className="py-16 text-center text-gray-400 text-sm">등록된 프로젝트가 없습니다.</div>
          ) : (
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="text-left text-xs font-bold text-gray-400 uppercase tracking-widest px-6 py-4">이미지</th>
                  <th className="text-left text-xs font-bold text-gray-400 uppercase tracking-widest px-4 py-4">제목</th>
                  <th className="text-left text-xs font-bold text-gray-400 uppercase tracking-widest px-4 py-4 hidden md:table-cell">카테고리</th>
                  <th className="text-left text-xs font-bold text-gray-400 uppercase tracking-widest px-4 py-4 hidden md:table-cell">연도</th>
                  <th className="px-6 py-4" />
                </tr>
              </thead>
              <tbody>
                {projects.map((p, i) => (
                  <tr key={p.id} className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${i === projects.length - 1 ? 'border-0' : ''}`}>
                    <td className="px-6 py-4">
                      {p.img
                        ? <img src={p.img} alt={p.title} className="w-14 h-10 object-cover rounded-lg bg-gray-100" />
                        : <div className="w-14 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-gray-300 text-xs">없음</div>
                      }
                    </td>
                    ...
                    <td className="px-4 py-4 hidden md:table-cell">
                      <span className="text-xs font-bold text-[#0FCD60] bg-green-50 px-2.5 py-1 rounded-full">{p.category}</span>
                    </td>
                    <td className="px-4 py-4 hidden md:table-cell text-sm text-gray-500">{p.year}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 justify-end">
                        <button onClick={() => onEdit(p)}
                          className="text-xs font-medium text-gray-600 hover:text-black px-3 py-1.5 rounded-lg hover:bg-gray-100 transition-colors">편집</button>
                        <button onClick={() => handleDelete(p.id)} disabled={deleting === p.id}
                          className="text-xs font-medium text-red-400 hover:text-red-600 px-3 py-1.5 rounded-lg hover:bg-red-50 transition-colors disabled:opacity-40">
                          {deleting === p.id ? '…' : '삭제'}
                        </button>
                      </div>
                    </td>
...
export default function App() {
  ...
  const handleLogout = async () => { await supabase.auth.signOut(); setUser(null); setView('site'); };
  ...
  const closeEditor = () => { setView(user ? 'admin' : 'site'); setEditingProject(null); };
