              ...
              {articles.map((a, i) => (
                <tr key={a.id} className="hover:bg-gray-50 transition-colors group"
                  ...
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-gray-400 mt-0.5">{a.author} · {a.country}</div>
                  </td>
...
function AdminDashboard({
  ...
  const [adminSubTab, setAdminSubTab] = useState<'projects' | 'articles'>('projects');
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-5xl mx-auto px-6 py-8">
          ...
          <div className="flex gap-1 p-1 bg-gray-100 rounded-xl mb-8 w-fit">
            {([['projects', 'PROJECTS'], ['articles', 'ARTICLES']] as const).map(([key, label]) => (
              <button key={key} onClick={() => setAdminSubTab(key as 'projects' | 'articles')}
...
function ArticleDetailPage({ article, onClose }: { article: Article; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[600] bg-white overflow-y-auto" style={{ animation: 'fadeIn 0.4s ease' }}>
      <button onClick={onClose} className="fixed top-6 left-6 z-10 flex items-center gap-2 bg-black/80 hover:bg-black text-white px-4 py-2.5 rounded-full text-sm font-medium shadow-lg transition-all cursor-pointer">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
        돌아가기
      </button>
      <div className="flex flex-col md:flex-row min-h-screen">
        ...
        <div className="w-full md:w-5/12 md:h-screen md:sticky md:top-0 relative flex flex-col justify-end p-10 text-white min-h-[50vh]"
          ...
          <div className="relative z-10">
            ...
            <h1 className="text-3xl md:text-5xl font-bold mb-3 leading-tight">{article.title}</h1>
            <p className="text-lg text-white/80">by {article.author}</p>
          </div>
        </div>
        {/* 오른쪽: 본문 */}
        <div className="w-full md:w-7/12 flex flex-col bg-[#fafafa]">
          <div className="flex-1 px-10 py-16 md:py-24 max-w-2xl">
            <div className="text-gray-800 text-lg leading-relaxed whitespace-pre-wrap font-light">
              {article.content || <span className="text-gray-300 italic">아직 작성된 내용이 없습니다.</span>}
            </div>
          </div>
          {/* 공유 */}
...
function ArticleEditorPage({
  ...
  const [form, setForm] = useState<Partial<Article>>(article ?? {
    slug: '', title: '', author: '', country: '', image_url: '', content: '', bg_color: '#e3d5ca', published: false,
  });
  const [uploading, setUploading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  ...
  const handleSave = async () => {
    if (!form.title?.trim() || !form.slug?.trim()) return alert('제목과 슬러그를 입력해주세요.');
    setSaving(true);
    await onSave(form);
    ...
    <div className="fixed inset-0 z-[400] bg-white flex flex-col overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-72 shrink-0 border-r border-gray-100 overflow-y-auto p-6 flex flex-col gap-5">
            ...
            {form.image_url
              ? <div className="relative w-full aspect-video rounded-xl overflow-hidden bg-gray-100 mb-2">
                  ...
                  <button type="button" onClick={() => set('image_url', '')}
                    className="absolute top-1.5 right-1.5 bg-black/60 text-white rounded-full w-6 h-6 text-xs flex items-center justify-center hover:bg-black cursor-pointer">✕</button>
                </div>
              : <div className="w-full aspect-video rounded-xl bg-gray-100 flex items-center justify-center text-gray-300 text-sm mb-2">미리보기 없음</div>
            }
            <label className="cursor-pointer block">
          ...
          {/* Slug */}
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">슬러그 * <span className="text-gray-300 font-normal normal-case">/article/slug</span></label>
            <input type="text" value={form.slug ?? ''} onChange={e => set('slug', e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-'))}
              placeholder="article-slug" className="w-full border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm font-mono" />
          </div>

          {/* Author */}
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">작가</label>
            <input type="text" value={form.author ?? ''} onChange={e => set('author', e.target.value)}
              placeholder="작가 이름" className="w-full border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm" />
          </div>

          {/* Country */}
          <div>
            <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">국가</label>
            <input type="text" value={form.country ?? ''} onChange={e => set('country', e.target.value.toUpperCase())}
              placeholder="KOREA" className="w-full border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm uppercase" />
          </div>
          ...
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
            <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">공개</span>
            <button
              type="button"
              onClick={() => set('published', !form.published)}
              className="relative w-11 h-6 rounded-full transition-colors cursor-pointer"
              style={{ backgroundColor: form.published ? '#0FCD60' : '#d1d5db' }}
            >
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-8">
            <textarea
              ...
              className="w-full h-full min-h-[500px] bg-transparent resize-none outline-none text-gray-800 text-lg leading-relaxed"
              style={{ fontFamily: 'inherit' }}
            />
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="fixed top-8 right-8 z-40 flex items-center gap-2 bg-black/90 hover:bg-black text-white px-5 py-3 rounded-full font-medium shadow-xl transition-all duration-300 hover:scale-105 active:scale-95"
      ...
      >
        ...
        <span className="text-sm tracking-wide">{isViewAll ? '캔버스 뷰' : '전체 보기'}</span>
      </button>
      ...
      {isAdmin && (
        <button
          ...
          className="fixed top-8 right-44 z-40 flex items-center gap-2 bg-[#0FCD60] text-white px-4 py-3 rounded-full font-medium shadow-xl transition-all hover:scale-105 active:scale-95"
            ...
            {articles.map(article => (
              ...
              >
                <div className="relative overflow-hidden w-[320px] h-[320px] shadow-lg group-hover:shadow-2xl transition-shadow duration-300 bg-white/10">
                  ...
                  {/* 아티클 번호 */}
                  <div className="absolute top-3 left-3 bg-black/60 text-white text-[10px] font-mono px-2 py-1 rounded backdrop-blur-sm">
                    #{String(article.id).padStart(3, '0')}
                  </div>
                  {article.content && (
                    <div className="absolute top-3 right-3 bg-black/70 text-white p-2 rounded-full backdrop-blur-md">
                ...
                <div className="mt-5 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] w-[320px]">
                  <h3 className="font-medium text-xl text-white">{article.title}</h3>
                  <p className="text-gray-400 mt-1 text-sm">by {article.author}</p>
                </div>
              </div>
            ))}
          </div>
...
export default function App() {
  ...
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'projects' | 'press' | 'contact' | 'article' | null>('projects');
  ...
  if (view === 'article-editor') {
    return (
      <ArticleEditorPage
        ...
        onClose={() => { setView('site'); setEditingArticle(undefined); setActiveTab('article'); }}
      />
    );
  }
    ...
    <>
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .site-logo { top: 2.875rem; left: 1.2rem; transform: translateY(-50%); }

          /* Glass nav — 모바일: 하단 고정, 1.4배 크기 */
          .glass-nav {
            top: auto !important;
          ...
          .glass-nav button { font-size: 1.1rem; padding: 0.55rem 1.1rem; }
          .nav-label { height: 25px; }

          /* site-logo & admin — 상단 유지 */
          .site-logo { top: 1.4rem; left: 1.2rem; }
          #admin-btn-wrapper { top: 1.4rem; right: 1.2rem; }

          /* 서브메뉴 — 하단 nav 위에 */
          #sub-menu { top: auto; bottom: 7rem; left: 1.4rem; font-size: 1rem; }
          .filter-label { height: 22px; }

          /* Gallery — 세로 스크롤 */
          #gallery-wrapper { align-items: flex-start; padding-top: 5vh; overflow-y: auto; overflow-x: hidden; }
          #gallery-wrapper.active { pointer-events: auto; }
          #gallery-track {
            flex-direction: column !important;
            align-items: center !important;
            gap: 8vh !important;
            padding: 2vh 0 12vh 0 !important;
            width: 100vw !important;
          }
          .gallery-img { height: auto; width: 82vw; max-width: 82vw; aspect-ratio: 4/3; object-fit: cover; }
          .gallery-info { opacity: 1 !important; transform: none !important; position: relative !important; margin-top: 1rem; }

          /* Intro slides — 모바일 세로 레이아웃 */
          .intro-slide-mobile-col {
            flex-direction: column !important;
            align-items: center !important;
          }
          ...
          /* Intro typo */
          .intro-typo { font-size: clamp(1.1rem, 4.5vw, 1.8rem); }
        ...
        <div ref={step4ContainerRef} className="intro-layer" style={{ pointerEvents: 'none' }}>
          ...
          <div id="ment3-wrap" style={{ position: 'absolute', top: 0, right: 0, width: '50%', height: '100%',
            ...
            <img ref={typo3Ref} src="/ment3.svg" alt="ment3"
              style={{ width: '150%', objectFit: 'contain', objectPosition: 'left center', filter: 'invert(1)' }} />
          </div>
        </div>
      ...
      {/* Article Modal */}
      <div className={`fullscreen-modal fixed inset-0 z-[150] transition-transform duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${activeTab === 'article' ? 'translate-y-0' : 'translate-y-full'}`}>
        {/* 닫기 버튼 — ArticlePage의 전체보기 버튼과 겹치지 않게 왼쪽에 배치 */}
        <button onClick={() => setActiveTab('projects')}
          className="fixed top-8 left-8 z-[200] flex items-center gap-2 bg-black/80 hover:bg-black text-white px-4 py-2.5 rounded-full text-sm font-medium shadow-lg transition-all cursor-pointer">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
          돌아가기
        </button>
