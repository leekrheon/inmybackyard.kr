...
function MailIconWithBadge({ active }: { active: boolean }) {
  ...
  const scrollToForm = () => {
    ...
    if (modal) modal.scrollTo({ top: window.innerHeight, behavior: 'smooth' });
  };
    ...
    >
      ...
      {badgeVisible && count > 0 && (
        <div style={{
          ...
          transform: popping ? 'scale(1.38)' : 'scale(1)',
          opacity: 1,
          transition: popping
            ? 'transform 0.2s cubic-bezier(0.34,1.56,0.64,1)'
            : 'transform 0.28s cubic-bezier(0.16,1,0.3,1)',
        }}>
          <span style={{ color: 'white', fontSize: 'clamp(9px, 1.8vw, 16px)', fontWeight: 400, fontFamily: '-apple-system, sans-serif', lineHeight: 1 }}>{count}</span>
        </div>
      )}
      {visible && (
        <p style={{ textAlign: 'center', fontSize: '0.72rem', color: '#bbb', marginTop: '0.6rem',
          fontFamily: 'Pretendard, -apple-system, sans-serif', letterSpacing: '0.04em' }}>
          tap to connect
        </p>
      )}
    </div>
...
export default function App() {
  ...
  return (
    <>
      ...
      {/* Contact Modal */}
      <div className={`fullscreen-modal fixed inset-0 z-[150] bg-[#fafafa] transition-transform duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${activeTab === 'contact' ? 'translate-y-0' : 'translate-y-full'}`}>
        <button onClick={() => setActiveTab('projects')}
          className="fixed z-[310] flex items-center justify-center w-9 h-9 rounded-full bg-black/80 hover:bg-black text-white shadow-lg transition-all cursor-pointer hover:scale-105"
          style={{ top: '2.875rem', right: '1.8rem', transform: 'translateY(-50%)' }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
