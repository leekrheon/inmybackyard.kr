...
// 마감일 실시간 계산 - 관리자가 선택한 날짜의 자정(00:00) 기준으로 종료
const getDday = (deadlineStr: string): { label: string; isExpired: boolean; date: Date | null } => {
  ...
  // 해당 날짜 자정 00:00:00 = 그 날이 시작되는 순간부터 유효, 다음날 자정부터 종료
  const deadline = new Date(dateMatch[1] + 'T00:00:00');
  // 다음날 00:00:00이 되면 종료 (= 선택한 날이 끝나는 자정)
...
function LoginPage() {
  ...
  const ADMIN_EMAIL = 'admin@catchcopy.kr'; // 관리자 이메일
          ...
          {!showAdminInput ? (
            ...
            <div className="mt-2 space-y-2">
              <input
                ...
                className="w-full rounded-lg px-3 py-2.5 text-sm border focus:outline-none focus:border-[#05D560] transition-colors"
                style={{ backgroundColor: dm ? D.bg : '#f9fafb', borderColor: dm ? D.border : '#e5e7eb', color: dm ? D.text : '#111' }}
              />
              <input
              ...
              <div className="flex gap-2">
                <button onClick={() => { setShowAdminInput(false); setAdminPw(''); }}
                  ...
                  style={{ borderColor: dm ? D.border : '#e5e7eb', color: D.muted }}>
                  취소
                </button>
...
function ActivitySection({ dm }: { dm: boolean }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
      <div className="px-5 pt-5 pb-3">
        <p className="text-sm font-bold" style={{ color: dm ? D.text : '#111' }}>내 활동</p>
      </div>
      {/* 탭 */}
      <div className="flex border-b" style={{ borderColor: dm ? D.border : '#f3f4f6' }}>
        {tabs.map(t => (
      ...
      {/* 내용 */}
      <div className="divide-y" style={{ borderColor: dm ? D.border : '#f9fafb' }}>
        {loading ? (
          ...
          posts.length === 0 ? (
            ...
            <button key={p.id} onClick={() => navigate('/catchcopy/community/post/' + p.id)}
              ...
              <div className="min-w-0">
                <p className="text-xs font-medium truncate" style={{ color: dm ? D.text : '#111' }}>{p.title}</p>
          ...
          comments.length === 0 ? (
            ...
            <button key={c.id} onClick={() => navigate('/catchcopy/community/post/' + c.post_id)}
              ...
              <p className="text-xs truncate" style={{ color: dm ? D.text : '#111' }}>{c.content}</p>
    ...
    return (
      <DarkWrapper className="min-h-screen flex flex-col items-center justify-center px-4 pt-12 gap-5">
        ...
        <div className="text-center">
          <p className="text-base font-bold mb-1">로그인이 필요해요</p>
          <p className="text-sm" style={{ color: D.muted }}>마이페이지를 이용하려면 로그인하세요.</p>
        </div>
  ...
  return (
    <DarkWrapper className="min-h-screen pt-12 pb-24 lg:pb-0">
      ...
      <div className="max-w-lg mx-auto px-4 py-8 space-y-4">
        {/* 프로필 */}
        <div className="rounded-2xl p-6" style={{ backgroundColor: dm ? D.card : 'white', border: dm ? ('1px solid ' + D.border) : '1px solid #f3f4f6' }}>
        ...
        {/* 개인정보 설정 */}
        <div className="rounded-2xl" style={{ backgroundColor: dm ? D.card : 'white', border: dm ? ('1px solid ' + D.border) : '1px solid #f3f4f6' }}>
        ...
        <div className="rounded-2xl" style={{ backgroundColor: dm ? D.card : 'white', border: dm ? ('1px solid ' + D.border) : '1px solid #f3f4f6' }}>
          <div className="flex items-center justify-between px-5 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium" style={{ color: dm ? D.text : '#111' }}>내 캐치</p>
        ...
        {isAdminMode() && (
          <div className="rounded-2xl" style={{ backgroundColor: dm ? D.card : 'white', border: dm ? ('1px solid ' + D.border) : '1px solid #f3f4f6' }}>
            <button onClick={() => navigate('/catchcopy/admin')}
              ...
              style={{ color: dm ? D.text : '#111' }}>
              <div>
                <p className="text-sm font-semibold">관리자 도구</p>
                <p className="text-[11px] mt-0.5" style={{ color: D.muted }}>브리프 · 지급 · 광고 · 분석 통합 관리</p>
              </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {user ? (
            <button onClick={() => navigate('/catchcopy/mypage')}
              ...
              style={{ color: dm ? D.text : '#374151', borderColor: user?.isAdmin ? KEY : (dm ? D.border : '#e5e7eb'), backgroundColor: dm ? D.hover : '#f9fafb' }}>
              {user.avatarUrl
...
function BriefCard({ brief, onClick, large }: { brief: Brief; onClick: () => void; large?: boolean }) {
  ...
  const isExpired = brief.status === '종료' || dday.isExpired;
  ...
  return (
    <motion.div whileHover={{ y: -2 }} whileTap={{ scale: 0.99 }} onClick={onClick}
      ...
      style={{ backgroundColor: dm ? D.card : 'white', border: dm ? ('1px solid ' + D.border) : '1px solid #f3f4f6',
      ...
      <div className={cn("relative overflow-hidden", large ? "h-52 sm:h-64" : "h-40 sm:h-48",
        ...
        {isExpired && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/30">
            <span className="text-white text-sm font-bold px-4 py-1.5 rounded-full bg-black/50 backdrop-blur-sm">브리프 종료</span>
          </div>
        )}
        {/* 뱃지들 */}
        <div className="absolute top-3 left-3 flex items-center gap-1.5">
          <span className={cn("text-[10px] font-semibold px-2 py-0.5 rounded-full",
            isExpired ? "bg-white/60 text-gray-500" : "bg-[#05D560] text-white")}>
            {isExpired ? '종료' : '진행중'}
          </span>
        </div>
      ...
      <div className="px-4 py-3 flex items-center justify-between" style={{ height: 52 }}>
        ...
        <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ color: D.muted, backgroundColor: dm ? D.hover : '#f9fafb' }}>{brief.category}</span>
      </div>
    </motion.div>
  );
}
...
function BriefPage() {
  ...
  const handleGoParticipate = () => {
    ...
    if (brief?.external_url && !hasVisitedUrl) { alert('자세히 알아보기 버튼을 눌러 상세 내용을 확인해주세요.'); return; }
    setMode('participate');
  };
  ...
  return (
    <div className="min-h-screen pt-12 pb-16" style={{ backgroundColor: dm ? D.bg : '#f9fafb' }}>
      {/* 풀스크린 헤더 이미지 */}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-8">
          <div className="flex items-center gap-2 mb-2">
            <span className={cn("text-[10px] font-semibold px-2.5 py-1 rounded-full",
              getDday(brief.deadline).isExpired || brief.status === '종료' ? "bg-white/20 text-white" : "bg-[#05D560] text-white")}>
              {getDday(brief.deadline).isExpired || brief.status === '종료' ? '종료' : '진행중'}
            </span>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        {[
          { label: '해결할 문제', content: brief.problem },
          { label: '타겟 고객', content: brief.target },
          { label: '캠페인 방향', content: brief.campaign_info },
        ].map(({ label, content }) => (
          <div key={label}>
            <p className="text-[10px] font-semibold uppercase tracking-widest mb-2" style={{ color: KEY }}>{label}</p>
            <div className="text-sm leading-relaxed" style={{ color: dm ? D.text : '#374151' }}
              dangerouslySetInnerHTML={{ __html: content ?? '' }} />
          </div>
        ))}

        {/* 메타 정보 */}
        <div className="rounded-2xl p-5 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-widest mb-1" style={{ color: D.muted }}>마감</p>
              <p className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>
            ...
            <div>
              ...
              <p className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>{brief.category}</p>
            </div>
          </div>
        </div>
        ...
        {(brief as any).pdf_url && (
          <a href={(brief as any).pdf_url} target="_blank" rel="noopener noreferrer"
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>상세 자료 다운로드</p>
        ...
        {brief.external_url && (
          <a href={brief.external_url} target="_blank" rel="noopener noreferrer"
            className="flex items-center justify-between w-full rounded-xl p-4 border transition-all hover:opacity-80"
            style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
            <p className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>자세히 알아보기</p>
            <ChevronRight size={16} style={{ color: D.muted }} />
          </a>
        )}
      </div>
  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen(v => !v)}
        className="w-full rounded-lg px-3 py-2.5 text-sm text-left focus:outline-none focus:border-[#05D560] transition-colors flex items-center justify-between border" style={{ backgroundColor: dm ? D.bg : '#f9fafb', borderColor: dm ? D.border : '#f3f4f6', color: dm ? D.text : '#111' }}>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute z-50 top-full mt-1.5 left-0 rounded-xl shadow-lg overflow-hidden border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#e5e7eb' }}
            style={{ width: 260 }}>
            {/* 월 네비게이션 */}
            <div className="flex items-center justify-between px-3 py-2.5 border-b" style={{ borderColor: dm ? D.border : '#f3f4f6' }}>
              ...
              <span className="text-xs font-semibold" style={{ color: dm ? D.text : '#374151' }}>{viewYear}년 {MONTHS[viewMonth]}</span>
              <button type="button" onClick={() => {
...
function BriefRichField({ value, onChange, placeholder, dm }: {
  ...
  const btnStyle = (active?: boolean) => ({
    ...
    backgroundColor: active ? '#05D560' : (dm ? '#2C2C35' : 'white'),
    color: active ? 'white' : (dm ? '#ccc' : '#374151'),
  });

  return (
    <div className="rounded-xl overflow-hidden border" style={{ borderColor: dm ? '#3a3a45' : '#e5e7eb' }}>
      {/* 서식 툴바 */}
      <div className="flex items-center gap-1 px-2 py-1.5 border-b" style={{ backgroundColor: dm ? '#2C2C35' : '#f9fafb', borderColor: dm ? '#3a3a45' : '#f3f4f6' }}>
      ...
      <div
        ...
        style={{
          minHeight: 80, color: dm ? '#e5e7eb' : '#374151',
          backgroundColor: dm ? '#17171C' : 'white',
        }}
      />
    </div>
  );
...
function BriefWritePage() {
  ...
  const [form, setForm] = useState({
    ...
    reward: '-', deadline: '', category: '식품', status: '진행중' as '진행중' | '종료',
    external_url: '',
  });
  ...
  const inputClass = "w-full rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-[#05D560] transition-colors border";
  const inputStyle = { backgroundColor: dm ? D.bg : "#f9fafb", borderColor: dm ? D.border : "#e5e7eb", color: dm ? D.text : "#111" };

  return (
    <div className="min-h-screen pt-12" style={{ backgroundColor: dm ? D.bg : '#f9fafb' }}>
      <div className="sticky top-12 z-40 border-b" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
        <div className="max-w-2xl mx-auto px-4 h-12 flex items-center justify-between">
          <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-xs font-medium transition-colors" style={{ color: D.muted }}>
            <ArrowLeft size={14} /> 취소
          </button>
          <span className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>브리프 등록</span>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* 이미지 업로드 */}
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wider mb-2" style={{ color: D.muted }}>브리프 이미지 <span className="normal-case font-normal">(권장: 800×500px, PNG/JPG)</span></p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <div className="flex p-0.5 rounded-lg gap-0.5" style={{ backgroundColor: dm ? D.hover : '#f3f4f6' }}>
          ...
          {[
            ...
            { key: 'title', label: '브리프 제목', placeholder: 'MZ세대를 위한 새로운 디카페인 커피 브랜드 런칭' },
          ...
          <div>
            <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">해결할 문제</label>
          ...
          <div>
            <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">캠페인 방향</label>
          ...
          <div>
            ...
            <div onClick={() => pdfRef.current?.click()}
              className="cursor-pointer rounded-xl border-2 border-dashed px-4 py-5 flex items-center gap-3 transition-all hover:border-[#05D560]/50"
              style={{ borderColor: pdfName ? KEY : (dm ? D.border : '#e5e7eb'), backgroundColor: dm ? D.bg : '#f9fafb' }}>
          ...
          <div>
            <label className="text-[11px] font-semibold uppercase tracking-wider block mb-1.5" style={{ color: D.muted }}>자세히 알아보기 URL <span className="font-normal normal-case">(선택)</span></label>
...
function WalletPage() {
  ...
  return (
    <div className="min-h-screen pt-12 pb-24 lg:pb-0" style={{ backgroundColor: dm ? D.bg : '#f9fafb' }}>
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        <button onClick={() => navigate('/catchcopy/market')}
          ...
          <div className="text-left">
            <p className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>캐치마켓</p>
        ...
        {/* 적립 내역 */}
        <div className="rounded-xl border overflow-hidden" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
          <div className="px-4 py-3 border-b" style={{ borderColor: dm ? D.border : '#f9fafb' }}>
            <p className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>적립 내역</p>
          </div>
          {allHistory.length === 0 ? (
            ...
            <div key={item.id} className="px-4 py-3.5 border-b flex items-center gap-3"
              style={{ borderColor: dm ? D.border : '#f9fafb' }}>
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold" style={{ color: dm ? D.text : '#111' }}>
...
function MarketPage() {
  ...
  return (
    <div className="min-h-screen pt-12 pb-24 lg:pb-0 flex flex-col items-center justify-center px-4"
      style={{ backgroundColor: dm ? D.bg : '#f9fafb' }}>
      <div className="text-center max-w-sm">
        <div className="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-5"
          style={{ backgroundColor: 'rgba(5,213,96,0.1)' }}>
          <span className="text-4xl">🎁</span>
        </div>
        <h2 className="text-xl font-bold mb-2" style={{ color: dm ? D.text : '#111' }}>캐치마켓</h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          {['스타벅스 아메리카노', '카카오페이 포인트', '배달의민족 쿠폰', 'GS25 모바일상품권'].map((item, i) => (
            <div key={i} className="rounded-xl p-3 text-center border opacity-40"
              ...
              <p className="text-[11px] font-medium" style={{ color: dm ? D.text : '#374151' }}>{item}</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-12">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        {/* 탭 */}
        <div className="flex gap-1 p-1 rounded-xl mb-5" style={{ backgroundColor: dm ? D.card : '#f3f4f6' }}>
          {[{ id: 'briefs', label: '브리프 관리' }, { id: 'payouts', label: '지급대기 ' + payouts.filter(p => p.status === 'pending').length }, { id: 'ads', label: '광고 관리' }, { id: 'news', label: '뉴스 관리' }, { id: 'analytics', label: '분석' }].map(t => (
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            ...
            <div key={i} className="rounded-xl border px-4 py-3 text-center" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
              <p className="text-xl font-bold" style={{ color: dm ? D.text : '#111' }}>{s.value}</p>
        ...
        {activeTab === 'payouts' && (
          <div>
            {payouts.length === 0 ? (
              ...
              <div className="space-y-2">
                {payouts.map(p => (
                  <div key={p.id} className="rounded-xl border p-4" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm font-medium mb-1" style={{ color: dm ? D.text : '#111' }}>"{p.copies?.content}"</p>
        ...
        {activeTab === 'analytics' && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>브리프 분석 대시보드</p>
              <button onClick={() => {
                // CSV Export
                ...
                a.href = url; a.download = 'catchcopy_report_' + new Date().toISOString().slice(0,10) + '.csv';
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-2">
                  {[
                    ...
                    { label: '총 카피수', value: analytics.reduce((s,a) => s + a.copyCount, 0).toLocaleString() },
                    ...
                    <div key={i} className="rounded-xl border p-3 text-center" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
                      <p className="text-lg font-bold" style={{ color: dm ? D.text : '#111' }}>{s.value}</p>
                      <p className="text-[11px] mt-0.5" style={{ color: D.muted }}>{s.label}</p>
                    </div>
                  ))}
                </div>
                ...
                {/* 브리프별 상세 */}
                {analytics.map((a) => (
                  <div key={a.id} className="rounded-xl border overflow-hidden" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
                    <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: dm ? D.border : '#f9fafb' }}>
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-semibold truncate" style={{ color: dm ? D.text : '#111' }}>{a.title}</p>
                        <p className="text-[11px] mt-0.5" style={{ color: D.muted }}>{a.company_name} · {a.category}</p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0 ml-3">
                        <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full"
                          style={{ backgroundColor: a.status === '진행중' ? 'rgba(5,213,96,0.1)' : 'rgba(156,163,175,0.15)', color: a.status === '진행중' ? KEY : D.muted }}>
                          {a.status}
                        </span>
                        <span className="text-sm font-bold" style={{ color: KEY }}>{a.adFee.toLocaleString()}원</span>
                      </div>
                    </div>
                    <div className="grid grid-cols-3 sm:grid-cols-5 divide-x" style={{ borderColor: dm ? D.border : '#f9fafb' }}>
                      {[
                        ...
                        { label: '카피 수', value: a.copyCount.toLocaleString() },
                        { label: '총 추천', value: a.totalVotes.toLocaleString() },
                      ].map((s, i) => (
                        <div key={i} className="px-3 py-2.5 text-center">
                          <p className="text-sm font-bold" style={{ color: dm ? D.text : '#111' }}>{s.value}</p>
                          <p className="text-[10px] mt-0.5" style={{ color: D.muted }}>{s.label}</p>
                        </div>
                      ))}
                    </div>
                    <div className="px-4 py-2 border-t flex items-center justify-between" style={{ borderColor: dm ? D.border : '#f9fafb', backgroundColor: dm ? D.hover : '#fafafa' }}>
                      <p className="text-[11px]" style={{ color: D.muted }}>광고비 산정: 조회수 {(a.views ?? 0).toLocaleString()} × 50원</p>
                      <p className="text-xs font-bold" style={{ color: dm ? D.text : '#111' }}>{a.adFee.toLocaleString()}원</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'ads' && (
          <div className="space-y-4">
            ...
            <div className="rounded-xl border p-4 flex items-center justify-between"
              style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
              <div>
                <p className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>광고 배너 ({ads.length}/5)</p>
            ...
            {ads.length > 0 && (
              <div>
                ...
                <div className="rounded-xl overflow-hidden border" style={{ borderColor: dm ? D.border : '#f3f4f6' }}>
                  <AdBanner />
                </div>
              </div>
            )}
                ...
                {ads.map((ad, idx) => {
                  ...
                  return (
                    <div key={ad.id} className="rounded-xl border overflow-hidden"
                      style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
                      {/* 썸네일 + 기본 정보 */}
                      <div className="flex items-center gap-3 p-3 border-b" style={{ borderColor: dm ? D.border : '#f9fafb' }}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <input type="url" defaultValue={ad.link_url ?? ''}
                            ...
                            className="w-full text-xs rounded-lg px-2.5 py-1.5 border focus:outline-none focus:border-[#05D560]"
                            style={{ backgroundColor: dm ? D.bg : '#f9fafb', borderColor: dm ? D.border : '#e5e7eb', color: dm ? D.text : '#111' }} />
                        </div>
                        {/* 순서 + 삭제 */}
    ...
    <div className="min-h-screen pt-12" style={{ backgroundColor: dm ? D.bg : '#f9fafb' }}>
      <div className="sticky top-12 z-40 border-b" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
        <div className="max-w-2xl mx-auto px-4 h-12 flex items-center justify-between">
          ...
          <span className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>브리프 수정</span>
          <button onClick={handleSubmit} disabled={submitting}
...
function WritePage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (title || content) {
          setForm({ title: title ?? '', category: category ?? '자유 게시판' });
          const trySet = () => {
  ...
  return (
    <div className="min-h-screen pt-12 flex flex-col" style={{ backgroundColor: dm ? D.bg : 'white' }}>
      <div className="sticky top-12 z-40 border-b" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
        <div className="max-w-2xl mx-auto px-4 h-12 flex items-center justify-between">
          <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-xs font-medium transition-colors" style={{ color: D.muted }}>
            <ArrowLeft size={14} /> 취소
          </button>
          <span className="text-sm font-semibold" style={{ color: dm ? D.text : '#111' }}>{isEdit ? '글 수정' : '새 글 작성'}</span>
          <div className="w-12" />
        </div>
      </div>
        ...
        <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-6 flex flex-col">
          <div className="relative mb-4">
            <select value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))}
              className="w-full rounded-lg px-3 py-2 text-sm appearance-none focus:outline-none focus:border-[#05D560] transition-colors border"
              style={{ backgroundColor: dm ? D.bg : '#f9fafb', borderColor: dm ? D.border : '#f3f4f6', color: dm ? D.text : '#374151' }}>
          ...
          <input type="text" value={form.title}
            ...
            style={{ borderColor: dm ? D.border : '#f3f4f6', color: dm ? D.text : '#111' }} />
          <div
            ref={editorRef}
            contentEditable
            suppressContentEditableWarning
            ...
            style={{ minHeight: 'calc(100vh - 380px)', color: dm ? D.text : '#374151' }}
          />
        </div>
      )}
...
function AppInner() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: dm ? D.bg : '#f9fafb', color: dm ? D.text : '#111' }}>
      <main className="pt-12 flex-1">
        ...
        {activeTab === 'home' && (
          <div className="overflow-y-scroll" style={{ height: 'calc(100vh - 48px)', scrollSnapType: 'y mandatory', scrollBehavior: 'smooth' }}>
            {/* 히어로 */}
            <section className="border-b transition-colors overflow-hidden" style={{ minHeight: 'calc(100vh - 48px)', scrollSnapAlign: 'start', scrollSnapStop: 'always', backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
              ...
              <div className="flex lg:hidden flex-col items-center justify-center text-center px-4 h-full" style={{ minHeight: 'calc(100vh - 48px)' }}>
                <div className="flex flex-col items-center gap-2 mb-5">
                  <img src="/ment1.svg" alt="돈이 되는 한 줄의 문장,"
                    ...
                    style={{ aspectRatio: '4905 / 721', filter: dm ? 'invert(1)' : 'none' }} />
                  <img src="/ment2.svg" alt="캐치카피"
                    className="w-full max-w-[220px] sm:max-w-xs"
                    style={{ aspectRatio: '3983 / 717', filter: dm ? 'invert(1)' : 'none' }} />
                </div>
                <p className="text-sm mb-8 leading-relaxed" style={{ color: D.muted }}>브랜드의 고민을 해결하는 한 줄의 카피.<br />지금 바로 도전하고 보상을 쟁취하세요.</p>
                <div className="flex items-center justify-center gap-3">
                  ...
                  <a href="/company.pdf" target="_blank" rel="noopener noreferrer"
                    className="text-sm font-medium px-6 py-3 rounded-lg border hover:bg-gray-50 transition-all"
                    style={{ color: dm ? D.text : '#374151', borderColor: dm ? D.border : '#e5e7eb' }}>
                    소개서 다운로드
                  </a>
                </div>
              ...
              <div className="hidden lg:flex items-stretch" style={{ minHeight: 'calc(100vh - 48px)' }}>
                ...
                <div className="flex-1 flex flex-col items-end justify-center pr-16 xl:pr-24 text-right">
                  <div className="flex flex-col items-end gap-3 mb-6">
                    <img src="/ment1.svg" alt="돈이 되는 한 줄의 문장,"
                      className="w-full"
                      style={{ maxWidth: 480, aspectRatio: '4905 / 721', filter: dm ? 'invert(1)' : 'none' }} />
                    <img src="/ment2.svg" alt="캐치카피"
                      className="w-full"
                      style={{ maxWidth: 380, aspectRatio: '3983 / 717', filter: dm ? 'invert(1)' : 'none' }} />
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <a href="/company.pdf" target="_blank" rel="noopener noreferrer"
                      className="text-sm font-medium px-7 py-3 rounded-lg border hover:bg-gray-50 transition-all"
                      style={{ color: dm ? D.text : '#374151', borderColor: dm ? D.border : '#e5e7eb' }}>
                      소개서 다운로드
                    </a>
                  </div>
            ...
            <section className="max-w-6xl mx-auto px-4 py-8">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-base font-bold" style={{ color: dm ? D.text : '#111' }}>참여 가능한 브리프</h3>
                <button onClick={() => navigate('/catchcopy/brief')}
        ...
        {activeTab === 'community' && (
          <div className="pb-24 lg:pb-0">
            ...
            {/* 모바일 상단 컨트롤 */}
            <div className="lg:hidden sticky top-12 z-30 border-b" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
            ...
            <div className="hidden lg:block max-w-4xl mx-auto px-4 py-6">
              <div className="grid grid-cols-4 gap-5">
                ...
                <aside className="col-span-1">
                  <div className="rounded-xl p-4 sticky top-16 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
                    ...
                    <div className="mt-5 pt-5 border-t" style={{ borderColor: dm ? D.border : '#f3f4f6' }}>
                        ...
                        <div className="space-y-2">
                          ...
                          {hotPosts.map((post, idx) => (
                            <motion.button
                              ...
                              <div className="min-w-0 flex-1">
                                <p className="text-[11px] leading-snug line-clamp-2 transition-colors" style={{ color: dm ? D.text : '#4b5563' }}>{post.title}</p>
                ...
                <div className="col-span-3">
                  <div className="flex items-center justify-between mb-4 gap-3">
                    <h2 className="text-base font-bold shrink-0" style={{ color: dm ? D.text : '#111' }}>{activeCategory}</h2>
                    <div className="flex items-center gap-2">
                      <div className="flex p-0.5 rounded-lg gap-0.5" style={{ backgroundColor: dm ? D.card : '#f3f4f6' }}>
                      ...
                      <div className="relative">
                        <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="검색"
                          ...
                          style={{ backgroundColor: dm ? D.bg : 'white', borderColor: dm ? D.border : '#e5e7eb', color: dm ? D.text : '#111' }} />
                        <Search className="absolute right-2 top-1/2 -translate-y-1/2" size={11} style={{ color: D.muted }} />
                      </div>
                      <button onClick={() => navigate('/catchcopy/community/write')}
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1.5 mb-1.5">
                                ...
                                <span className="text-[10px] font-medium px-1.5 py-0.5 rounded" style={{ color: D.muted, backgroundColor: dm ? D.hover : '#f9fafb' }}>{post.category}</span>
                              </div>
                              <h4 className="text-sm font-semibold mb-1.5 leading-snug" style={{ color: dm ? D.text : '#111' }}>{post.title}</h4>
                            ...
                            <div className="shrink-0 flex flex-col items-center gap-0.5 text-[11px] min-w-[36px] text-center" style={{ color: D.muted }}>
                              <ThumbsUp size={11} />
                              <span className="font-semibold" style={{ color: dm ? D.text : '#374151' }}>{post.likes}</span>
                            </div>
                          </div>
                        </motion.div>
                      ))}
                ...
                <div>
                  {filteredPosts.length === 0 ? (
                    ...
                    <motion.div key={post.id} whileTap={{ scale: 0.99 }}
                      ...
                      style={{ borderColor: dm ? D.border : '#f3f4f6', backgroundColor: dm ? D.bg : 'white' }}>
                      <div className="px-4 py-4">
                        <div className="flex items-center gap-2.5 mb-2.5">
                          <Avatar name={post.avatar} size={8} imageUrl={user?.name === post.author ? (user?.avatarUrl ?? post.avatar_url ?? undefined) : (post.avatar_url ?? undefined)} />
                          <div>
                            <div className="flex items-center gap-1.5">
                              <span className="text-xs font-semibold" style={{ color: dm ? D.text : '#111' }}>{post.author}</span>
                            ...
                            <div className="flex items-center gap-1.5 text-[11px]" style={{ color: D.muted }}>
                              ...
                              <span>·</span>
                              <span className="px-1.5 py-0.5 rounded-full text-[10px]" style={{ backgroundColor: dm ? D.card : '#f9fafb' }}>{post.category}</span>
                            </div>
                          </div>
                        </div>
                        <h4 className="text-sm font-semibold mb-1 leading-snug" style={{ color: dm ? D.text : '#111' }}>{post.title}</h4>
                        {post.content && (
                          <p className="text-xs leading-relaxed line-clamp-2 mb-3" style={{ color: dm ? D.muted : '#6b7280' }}
      ...
      <footer className="hidden lg:block border-t py-6 mt-8 transition-colors" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
...
function NewsAdminTab({ dm }: { dm: boolean }) {
  ...
  return (
    <div className="space-y-4">
      {/* URL 입력 */}
      <div className="rounded-xl border p-4" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
        <p className="text-sm font-semibold mb-3" style={{ color: dm ? D.text : '#111' }}>뉴스 기사 추가</p>
        ...
        <div className="flex gap-2">
          <input type="url" value={urlInput} onChange={e => setUrlInput(e.target.value)}
            ...
            className="flex-1 rounded-lg px-3 py-2.5 text-sm border focus:outline-none focus:border-[#05D560] transition-colors"
            style={{ backgroundColor: dm ? D.bg : '#f9fafb', borderColor: dm ? D.border : '#e5e7eb', color: dm ? D.text : '#111' }} />
        ...
        <div className="space-y-2">
          {articles.map(a => (
            <div key={a.id} className="rounded-xl border overflow-hidden flex items-center gap-3 p-3"
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate" style={{ color: dm ? D.text : '#111' }}>{a.title || a.url}</p>
...
function MediaPage() {
  ...
  return (
    <div className="min-h-screen pt-12 pb-24 lg:pb-0" style={{ backgroundColor: dm ? D.bg : '#f9fafb' }}>
      {/* 헤더 */}
      <div className="border-b" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
        <div className="max-w-5xl mx-auto px-4 py-8">
          <p className="text-[11px] font-semibold uppercase tracking-widest mb-2" style={{ color: KEY }}>Media</p>
          <h1 className="text-2xl font-bold" style={{ color: dm ? D.text : '#111' }}>Press & News</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {articles.map(a => (
              <motion.a key={a.id} href={a.url} target="_blank" rel="noopener noreferrer"
                ...
                <div className="relative overflow-hidden" style={{ height: 180 }}>
                  {a.image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-4xl"
                      style={{ backgroundColor: dm ? D.hover : '#f3f4f6' }}>
                      📰
                    </div>
                  )}
                  {/* 출처 배지 */}
                ...
                {/* 내용 */}
                <div className="p-4">
                  <h3 className="text-sm font-bold leading-snug mb-2 line-clamp-2" style={{ color: dm ? D.text : '#111' }}>
                    {a.title || a.url}
                  </h3>
                  {a.description && (
                    <p className="text-[12px] leading-relaxed line-clamp-2 mb-3" style={{ color: dm ? D.muted : '#6b7280' }}>
                      {a.description}
                    </p>
                  )}
...
function ContactPage() {
  ...
  const inputClass = "w-full rounded-xl px-4 py-3 text-sm border focus:outline-none focus:border-[#05D560] transition-colors";
  const inputStyle = { backgroundColor: dm ? D.bg : '#f9fafb', borderColor: dm ? D.border : '#e5e7eb', color: dm ? D.text : '#111' };
    ...
    <div className="min-h-screen pt-12 pb-24 lg:pb-0" style={{ backgroundColor: dm ? D.bg : '#f9fafb' }}>
      ...
      <div className="border-b" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
        <div className="max-w-3xl mx-auto px-4 py-10">
          ...
          <h1 className="text-2xl font-bold mb-1" style={{ color: dm ? D.text : '#111' }}>문의하기</h1>
      ...
      <div className="max-w-3xl mx-auto px-4 py-10">
        {sent ? (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            ...
            <h2 className="text-lg font-bold mb-2" style={{ color: dm ? D.text : '#111' }}>문의가 접수되었습니다!</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="rounded-2xl p-6 border" style={{ backgroundColor: dm ? D.card : 'white', borderColor: dm ? D.border : '#f3f4f6' }}>
                ...
                {[
                  ...
                  <div key={item.label} className="mb-3">
                    ...
                    <p className="text-sm font-medium" style={{ color: dm ? D.text : '#111' }}>{item.value}</p>
                  </div>
                ))}
              </div>
            </div>
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session?.user) {
        ...
        const ADMIN_EMAIL = 'admin@catchcopy.kr';
