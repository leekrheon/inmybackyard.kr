...
const CATEGORIES = ['전체', '아이디어 제안', '비하인드 캠페인', '공지사항'];
...
const getVoterKey = () => {
  let k = localStorage.getItem('ib_voter_key');
  if (!k) {
    ...
    k = uid + '_' + fpHash;
    localStorage.setItem('ib_voter_key', k);
    // sessionStorage에도 백업 (탭 새로고침 시 검증용)
    sessionStorage.setItem('ib_vk_backup', k);
  }
  // voter_key 변조 감지: sessionStorage 백업과 다르면 복원
  const backup = sessionStorage.getItem('ib_vk_backup');
  if (backup && backup !== k) {
    // 변조 감지 - sessionStorage가 있으면 복원
    localStorage.setItem('ib_voter_key', backup);
    return backup;
  }
  return k;
};
...
function LoginPage() {
  ...
  const handleLogin = (provider: 'naver' | 'kakao') => {
    ...
    setUser(profile);
    localStorage.setItem('ib_user', JSON.stringify(profile));
    navigate(-1);
  };
  ...
  const handleAdminLogin = async () => {
    ...
    try {
      ...
      setUser(userProfile);
      localStorage.setItem('ib_user', JSON.stringify(userProfile));
      navigate('/imbykorea/mypage');
    } catch {
      ...
      <div className="divide-y" style={{ borderColor: D.border }}>
        {loading ? (
          ...
          posts.length === 0 ? (
            ...
            <button key={p.id} onClick={() => navigate('/imbykorea/community/post/' + p.id)}
          ...
          comments.length === 0 ? (
            ...
            <button key={c.id} onClick={() => navigate('/imbykorea/community/post/' + c.post_id)}
  ...
  if (!user) {
    return (
      <DarkWrapper className="min-h-screen flex flex-col items-center justify-center px-4 pt-12 gap-5">
        ...
        <button onClick={() => navigate('/imbykorea/login')}
      ...
      <div className="max-w-lg mx-auto px-4 py-8 space-y-4">
        ...
        <div className="rounded-2xl" style={{ backgroundColor: D.card, border: '1px solid ' + D.border }}>
          <div className="flex items-center justify-between px-5 py-4">
            ...
            <button onClick={() => navigate('/imbykorea/wallet')}
        ...
        {isAdminMode() && (
          <div className="rounded-2xl" style={{ backgroundColor: D.card, border: '1px solid ' + D.border }}>
            <button onClick={() => navigate('/imbykorea/admin')}
...
function MobileTabBar() {
  ...
  const activeTab = path.startsWith('/imbykorea/brief') && !path.includes('/admin') ? 'brief'
    : path.startsWith('/imbykorea/community') ? 'community'
    : path.startsWith('/imbykorea/media') ? 'media'
    : path.startsWith('/imbykorea/contact') ? 'contact'
    : path.startsWith('/imbykorea/mypage') ? 'mypage'
    : 'home';

  const tabs = [
    { id: 'home', label: '홈', path: '/imbykorea',
    ...
    { id: 'brief', label: 'WORK', path: '/imbykorea/brief',
    ...
    { id: 'community', label: 'COMMUNITY', path: '/imbykorea/community',
    ...
    { id: 'media', label: 'MEDIA', path: '/imbykorea/media',
    ...
    { id: 'mypage', label: 'MY', path: '/imbykorea/mypage',
...
function Header() {
  ...
  const dm = darkMode;
  const activeTab = location.pathname.startsWith('/imbykorea/brief') ? 'brief'
    : location.pathname.startsWith('/imbykorea/community') ? 'community'
    : location.pathname.startsWith('/imbykorea/media') ? 'media'
    : location.pathname.startsWith('/imbykorea/contact') ? 'contact' : 'home';
    ...
    <header className="fixed top-0 left-0 right-0 z-50 border-b transition-colors"
      ...
      <div className="max-w-6xl mx-auto px-4 h-12 flex items-center justify-between">
        <div className="flex items-center gap-5">
          <button onClick={() => navigate('/imbykorea')} className="flex items-center">
          ...
          <nav className="hidden md:flex items-center gap-0.5">
            {[{ id: 'brief', label: 'WORK' }, { id: 'community', label: 'COMMUNITY' }, { id: 'media', label: 'MEDIA' }, { id: 'contact', label: 'CONTACT' }].map(t => (
              <button key={t.id} onClick={() => navigate(`/imbykorea/${t.id}`)}
                className="px-3 py-1.5 rounded-lg text-xs transition-all"
                style={{ fontFamily: "'Calisto MT', 'Calisto', Georgia, serif", fontWeight: 400, letterSpacing: '0.04em', color: activeTab === t.id ? KEY : dm ? D.muted : '#6b7280', backgroundColor: activeTab === t.id ? 'rgba(5, 213, 96, 0.08)' : 'transparent' }}>
                {t.label}
              </button>
            ))}
          </nav>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {user ? (
            <button onClick={() => navigate('/imbykorea/mypage')}
      ...
      <div className={cn("relative overflow-hidden", large ? "h-52 sm:h-64" : "h-40 sm:h-48",
        ...
        {isExpired && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/30">
            <span className="text-white text-sm font-bold px-4 py-1.5 rounded-full bg-black/50 backdrop-blur-sm">포트폴리오 CLOSED</span>
          </div>
        )}
        {/* 뱃지들 */}
...
function BriefPage() {
  ...
  const handleSubmitCopy = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('ib_catches', String(newCatches));
      setHasCopy(true);
      setNewCopy('');
  ...
  return (
    <div className="min-h-screen pt-12 pb-16" style={{ backgroundColor: D.bg }}>
      ...
      <div className={cn("relative w-full", !brief.image_url && brief.bg_color)} style={{ height: '40vh', minHeight: 220 }}>
        ...
        <button onClick={() => navigate('/imbykorea/brief')}
...
function BriefWritePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(false);
    if (!error) navigate('/imbykorea/brief');
  ...
  return (
    <div className="min-h-screen pt-12 pb-24 lg:pb-0" style={{ backgroundColor: D.bg }}>
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        {/* 캐치마켓 배너 */}
        <button onClick={() => navigate('/imbykorea/market')}
...
function AdminPage() {
  ...
  const [adminMode, setAdminMode] = useState(() => localStorage.getItem('ib_admin') === '1');
  const [payouts, setPayouts] = useState<any[]>([]);
  const [activeTab, setActiveTab] = useState<'briefs' | 'payouts' | 'ads' | 'analytics' | 'contacts'>('briefs');
  ...
  const toggleAdminMode = () => {
    ...
    setAdminMode(next);
    localStorage.setItem('ib_admin', next ? '1' : '0');
  };
    ...
    <div className="min-h-screen pt-12" style={{ backgroundColor: D.bg }}>
      <div className="sticky top-12 z-40 border-b" style={{ backgroundColor: D.card, borderColor: D.border }}>
        <div className="max-w-5xl mx-auto px-4 h-12 flex items-center justify-between">
          ...
          <button onClick={() => navigate('/imbykorea/admin/brief/write')}
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-1 p-1 rounded-xl mb-5" style={{ backgroundColor: D.card }}>
          {[{ id: 'briefs', label: '프로젝트 관리' }, { id: 'payouts', label: '지급대기 ' + payouts.filter(p => p.status === 'pending').length }, { id: 'ads', label: '광고 관리' }, { id: 'news', label: '뉴스 관리' }, { id: 'analytics', label: '분석' }, { id: 'contacts', label: '문의 ' + contacts.length }].map(t => (
            <button key={t.id} onClick={() => { setActiveTab(t.id as any); if (t.id === 'analytics') loadAnalytics(); if (t.id === 'contacts') loadContacts(); }}
        ...
        {loading ? <Spinner /> : briefs.length === 0 ? (
          <div className="text-center py-20 text-gray-300">
            ...
            <p className="text-xs">등록된 포트폴리오가 없습니다.</p>
          </div>
        ) : (
              ...
              <div key={brief.id} className="rounded-xl border transition-all overflow-hidden" style={{ backgroundColor: D.card, borderColor: D.border }}>
                <div className="flex items-center gap-3 p-4">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    {/* 수정 */}
                    <button onClick={() => navigate(`/imbykorea/admin/brief/edit/${brief.id}`)}
        ...
        {activeTab === 'contacts' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-semibold" style={{ color: D.text }}>문의 내용 ({contacts.length}건)</p>
              <button onClick={loadContacts} className="text-xs font-medium px-3 py-1.5 rounded-lg border transition-all" style={{ borderColor: D.border, color: D.muted }}>새로고침</button>
            </div>
            {contactsLoading ? <Spinner /> : contacts.length === 0 ? (
              ...
              <div key={c.id} className="rounded-xl border p-4 space-y-2" style={{ backgroundColor: D.card, borderColor: D.border }}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className="text-sm font-bold" style={{ color: D.text }}>{c.name}</span>
                      {c.company && <span className="text-[11px] px-2 py-0.5 rounded-full" style={{ backgroundColor: D.hover, color: D.muted }}>{c.company}</span>}
                    </div>
                    <a href={`mailto:${c.email}`} className="text-xs font-medium" style={{ color: KEY }}>{c.email}</a>
                  </div>
                  <span className="text-[11px] whitespace-nowrap shrink-0" style={{ color: D.muted }}>{timeAgo(c.created_at)}</span>
                </div>
                <div className="rounded-lg p-3 text-sm leading-relaxed" style={{ backgroundColor: D.bg, color: D.text }}>
                  {c.message}
                </div>
              </div>
            ))}
...
function BriefEditPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(false);
    if (!error) navigate('/imbykorea/admin');
  ...
  return (
    <div className="min-h-screen pt-12" style={{ backgroundColor: D.bg }}>
      <div className="sticky top-12 z-40 border-b" style={{ backgroundColor: D.card, borderColor: D.border }}>
        <div className="max-w-2xl mx-auto px-4 h-12 flex items-center justify-between">
          <button onClick={() => navigate('/imbykorea/admin')} className="flex items-center gap-1.5 text-xs font-medium transition-colors" style={{ color: D.muted }}>
...
function WritePage() {
  ...
  const handleSubmit = async () => {
    ...
    if (isEdit) {
      ...
      navigate('/imbykorea/community/post/' + postId);
    } else {
      ...
      if (!error) {
        ...
        navigate(data ? '/imbykorea/community/post/' + data.id : '/imbykorea/community');
      } else {
...
const isAdminMode = () => {
  ...
  try {
    const userStr = localStorage.getItem('ib_user');
...
function PostDetail() {
  ...
  const handleDeletePost = async () => {
    ...
    if (!error) navigate('/imbykorea/community');
  ...
  return (
    <div className="min-h-screen pt-12" style={{ backgroundColor: D.bg }}>
      <div className="max-w-2xl mx-auto px-4 py-6">
        <button onClick={() => navigate('/imbykorea/community')}
            ...
            <div className="flex items-center justify-between pb-4 border-b border-gray-50">
              ...
              <div className="flex items-center gap-3">
                ...
                {((user && user.name === post.author) || isAdminMode()) && (<>
                  <button onClick={() => navigate(`/imbykorea/community/edit/${post.id}`)}
...
function AppInner() {
  ...
  const activeTab = location.pathname.startsWith('/imbykorea/brief') ? 'brief'
    : location.pathname.startsWith('/imbykorea/community') ? 'community'
    : location.pathname.startsWith('/imbykorea/wallet') ? 'wallet'
    : location.pathname === '/imbykorea/mypage' ? 'mypage' : 'home';
          ...
          <div className="overflow-y-scroll" style={{ height: 'calc(100vh - 48px)', scrollSnapType: 'y mandatory', scrollBehavior: 'smooth' }}>
            ...
            <section className="border-b transition-colors" style={{ backgroundColor: D.card, borderColor: D.border }}>
              <div className="max-w-6xl mx-auto px-4 py-5 grid grid-cols-3" style={{ divideColor: dm ? D.border : '#f3f4f6' }}>
                {[
                  { label: '진행 중인 프로젝트', value: `${stats.activeBriefs}개`, icon: <Zap size={13} /> },
            ...
            <section className="max-w-6xl mx-auto px-4 py-8">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-base font-bold" style={{ color: D.text }}>진행 중인 프로젝트</h3>
                <button onClick={() => navigate('/imbykorea/brief')}
              ...
              {briefsLoading ? <Spinner /> : (
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  {homeBriefs.map((brief, i) => (
                    <BriefCard key={brief.id} brief={brief} large={i === 0} onClick={() => navigate(`/imbykorea/brief/${brief.id}`)} />
                  ))}
                </div>
              )}
            </section>
        ...
        {activeTab === 'brief' && (
          <div className="max-w-6xl mx-auto px-4 py-6 pb-24 lg:pb-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-5">
              <div>
                <h2 className="text-xl font-bold mb-0.5">포트폴리오 아카이브</h2>
                <p className="text-xs text-gray-400">imby의 작업물을 소개합니다.</p>
              </div>
              {/* 한 줄 필터 바 */}
              <div className="flex items-center gap-2">
                {/* 상태 필터 */}
            ...
            {briefsLoading ? <Spinner /> : filteredBriefs.length === 0 ? (
              <div className="text-center py-20 text-gray-300">
                <Megaphone size={32} className="mx-auto mb-3" />
                <p className="text-xs">해당하는 포트폴리오가 없습니다.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {/* 상위 2개 대형 */}
                {filteredBriefs.length >= 1 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {filteredBriefs.slice(0, 2).map(brief => (
                      <BriefCard key={brief.id} large brief={brief} onClick={() => navigate(`/imbykorea/brief/${brief.id}`)} />
                    ))}
                  </div>
                )}
                {filteredBriefs.length > 2 && (
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    {filteredBriefs.slice(2).map(brief => (
                      <BriefCard key={brief.id} brief={brief} onClick={() => navigate(`/imbykorea/brief/${brief.id}`)} />
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="lg:hidden sticky top-12 z-30 border-b" style={{ backgroundColor: D.card, borderColor: D.border }}>
              ...
              <div className="flex items-center justify-between px-4 py-2 gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => navigate('/imbykorea/community/write')}
                        ...
                        <div className="space-y-2">
                          ...
                          {hotPosts.map((post, idx) => (
                            <motion.button
                              ...
                              onClick={() => navigate('/imbykorea/community/post/' + post.id)}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="rounded-2xl p-6 border" style={{ backgroundColor: D.card, borderColor: D.border }}>
                ...
                {[
                  { label: 'Email', value: 'support@inmybackyard.kr' },
...
export default function App() {
  const [user, setUser] = useState<UserProfile | null>(() => {
    try { const s = localStorage.getItem('ib_user'); return s ? JSON.parse(s) : null; } catch { return null; }
  });
  const [darkMode, setDarkMode] = useState(() => localStorage.getItem('ib_dark') !== '0');
    ...
    supabase.from('user_catches').select('catches').eq('voter_key', vk).single()
      .then(({ data }) => {
        if (data) {
          ...
          localStorage.setItem('ib_catches', String(data.catches));
        } else {
          setCatches(0);
          localStorage.setItem('ib_catches', '0');
        }
      });
  }, []);
  ...
  const toggleDarkMode = () => {
    setDarkMode(d => {
      localStorage.setItem('ib_dark', d ? '0' : '1');
      return !d;
    });
  };
    ...
    <AppContext.Provider value={{ user, setUser, darkMode, toggleDarkMode, catches, setCatches, ads, setAds }}>
      <div style={{ minHeight: '100vh', backgroundColor: D.bg, color: D.text, transition: 'background-color 0.3s, color 0.3s' }}>
        <BrowserRouter>
          ...
          <MobileTabBar />
          <Routes>
            <Route path="/imbykorea/login" element={<LoginPage />} />
            <Route path="/imbykorea/mypage" element={<MyPage />} />
            <Route path="/imbykorea/wallet" element={<WalletPage />} />
            <Route path="/imbykorea/market" element={<MarketPage />} />
            <Route path="/imbykorea/admin" element={<AdminPage />} />
            <Route path="/imbykorea/admin/brief/write" element={<BriefWritePage />} />
            <Route path="/imbykorea/admin/brief/edit/:briefId" element={<BriefEditPage />} />
            <Route path="/imbykorea/brief/:briefId" element={<BriefPage />} />
            <Route path="/imbykorea/community/post/:postId" element={<PostDetail />} />
            <Route path="/imbykorea/community/write" element={<WritePage />} />
            <Route path="/imbykorea/community/edit/:postId" element={<WritePage />} />
            <Route path="/imbykorea/media" element={<MediaPage />} />
            <Route path="/imbykorea/contact" element={<ContactPage />} />
            <Route path="/imbykorea/*" element={<AppInner />} />
